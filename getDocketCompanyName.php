...
if($_REQUEST['clientType']!=2){
  ...
  $contperQuery=GetPageRecord('*','contactPersonMaster',' 1 and corporateId="'.$userInfopost['id'].'"'); 
  $contactPerData=mysqli_fetch_array($contperQuery); 
  $contactPersonN = $contactPerData['contactPerson'];
  $phone = decode($contactPerData['phone']);
  $email = decode($contactPerData['email']);
  $division = $contactPerData['division'];
  $designation = $contactPerData['designation'];




  ?>
  <div class="selectParentList" style="padding-left:10px;" onclick="selectCorporateCompany('<?php echo strip($userInfopost['name']); ?>','<?php echo strip($email); ?>','<?php echo strip($contactPersonN); ?>','<?php echo strip($phone); ?>','<?php echo encode(strip($userInfopost['id'])); ?>','<?php echo getUserName($userInfopost['OpsAssignTo']); ?>','<?php echo encode($userInfopost['OpsAssignTo']); ?>','<?php echo $nationality; ?>','<?php echo $languageId; ?>','<?php echo getUserName($userInfopost['assignTo']); ?>','<?php echo $marketType; ?>','<?php echo $nationId ?>','<?php echo $marketId ?>','<?php echo $userInfopost['tourType']; ?>','<?php echo $division; ?>','<?php echo $designation; ?>');<?php if($userInfopost['companyCategory']=='1'){ ?>$('#queryPriority').val('3');<?php } ?>fillagentname('<?php echo getPrimaryNameCompany($userInfopost['id'],'corporate'); ?>');">
...
$marketId = $marketName['id'];
 ?>
  ...
  <div class="selectParentList" style="padding-left:10px;" onclick="selectCorporateCompany('<?php echo strip($userInfopost['firstName'].' '.$userInfopost['lastName']); ?>','<?php echo getPrimaryEmail($userInfopost['id'],'contacts'); ?>','<?php echo getPrimaryPhone($userInfopost['id'],'contacts'); ?>','<?php echo encode(strip($userInfopost['id'])); ?>','<?php echo getUserName($userInfopost['assignTo']);  ?>','<?php echo encode($userInfopost['assignTo']); ?>','<?php echo $nationality; ?>','<?php echo $userInfopost['language']; ?>','<?php echo getUserName($userInfopost['assignTo']); ?>','<?php echo $marketType; ?>','<?php echo $nationId ?>','<?php echo $marketId ?>','<?php echo $userInfopost['tourType'] ?>');fillagentname('<?php echo strip($userInfopost['firstName'].' '.$userInfopost['lastName']); ?>');">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
