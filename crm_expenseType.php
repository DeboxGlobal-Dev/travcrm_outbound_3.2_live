...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=State','600px','auto');" />
	<?php } ?>
	</div>
...
<input name="action" id="action" type="hidden" value="deleteExpenseType" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
  <thead>
    <tr>
      ...
      <th width="3%" align="center" valign="middle" class="header" ><?php if($editpermission==1){ ?>
          <input type="checkbox" id="checkAll"  name="checkedAll" onclick="checkallbox();" />
        <?php } ?>
          <label for="checkAll"><span></span>&nbsp;</label></th>
      <th align="left" class="header" >Expense Type </th>
      <th align="left" class="header" >Expense Head</th>
...
$rs=GetRecordList($select,'expenseTypeMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
$select1='*';  
$where1='id='.$resultlists['expenseHeadId'].''; 
...
?>
    <tr>
      <td width="1%" align="left" class="header"><?= $no; ?></td>    
      <td width="1%" align="center" valign="middle"><?php if($editpermission==1){ ?>
        ...
        <?php } ?></td>
      <td width="9%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div></td>
      <td width="9%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $country_result['name']; ?></div></td>
      <td width="12%" align="left"><?php $select=''; 
$where2=''; 
$rs2='';  
...
echo $userss['firstName'].' '.$userss['lastName'];?>
	...
	<td width="4%" align="left"><?php if($resultlists['status']==1){?><div style=" width: fit-content; color: green;"><?php echo 'Active';?></div><?php } else { ?><div style=" width: fit-content; color: red; "><?php echo 'In Active';?></div><?php }  ?>
	</td>
    </tr>
    <?php $no++; } ?>
  </tbody>
