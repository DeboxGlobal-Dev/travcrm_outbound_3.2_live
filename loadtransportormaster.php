...
while($resListing=mysqli_fetch_array($rs)){  
	...
	while($PriceresListing=mysqli_fetch_array($rs23)){
		...
		<div class="roompricelistmain" style="overflow:auto">
		...
		while($dmcroommastermain=mysqli_fetch_array($rs22)){
			...
			<?php }else{ ?>
			<td width="7%" align="left" bgcolor="#f7f7f7"><strong>CostType</strong></td>
			<?php if($dmcroommastermain['transferCostType'] == 3) { ?>
			<td width="5%" align="center" bgcolor="#f7f7f7"><strong>Per&nbsp;KM&nbsp;Cost</strong></td>
			<td width="5%" align="center" bgcolor="#f7f7f7"><strong>Distance</strong></td>
			<?php }elseif($dmcroommastermain['transferCostType'] == 1){  ?>
			<td width="5%" align="center" bgcolor="#f7f7f7"><strong>Per&nbsp;Day&nbsp;Cost</strong></td>
			<?php }else{  ?>
			...
			<?php }else{ ?>
			<td align="left"><?php echo transferCostType($dmcroommastermain['transferCostType']); ?></td>
			<td align="center"><?php echo $cur.'&nbsp;'.strip($dmcroommastermain['vehicleCost']); ?></td>
			<?php if($dmcroommastermain['transferCostType'] == 3){ ?>
			<td align="center"><?php echo strip($dmcroommastermain['distance']); ?> KM</td>
			<?php } ?>
			...
			<td align="center"><a onClick="alertspopupopen('action=editdmctransferrate&sectionId=<?php echo $dmcroommastermain['id']; ?>','1200px','auto');"><i class="fa fa-pencil" aria-hidden="true" style="font-size: 15px;padding: 5px 10px;border-radius: 5px;cursor:pointer;" ></i></a>
