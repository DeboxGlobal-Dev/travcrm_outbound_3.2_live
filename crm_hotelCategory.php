...
<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=HotelCategory','600px','auto');" />
<?php } ?>
</div>
...
<div id="pagelisterouter" style="padding:0 30px;" >
...
<th width="40%" align="left" class="header" >Hotel Category</th>
<th  align="left" class="header" >Upload&nbsp;Keyword</th>
...
$where='where hotelCategory!="" and deletestatus=0 order by id desc'; 
$page=$_GET['page'];
 
$targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 
$rs=GetRecordList($select,_HOTEL_CATEGORY_MASTER_,$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
while($resultlists=mysqli_fetch_array($rs[0])){ 
    ...
    <td  align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['hotelCategory']." Star"; ?></div></td>
	<td  align="left" ><?php echo $resultlists['uploadKeyword']; ?></td>
	<td align="left"><?php 
	$select=''; 
	$where2=''; 
	$rs2='';  
