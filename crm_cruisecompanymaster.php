...
<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Cruise&nbsp;Company&nbsp;Master','600px','auto');" />
	<?php } ?>
	</div>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add New Cruise Company"  onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','700px','auto');" /></td>  <?php } ?>
      </tr>
...
<input name="action" id="action" type="hidden" value="deletepackagecruisecompany" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
     ...
     <th align="left" class="header" >Cruise&nbsp;Company</th>
     <!-- <th align="left" class="header" >Ferry&nbsp;Type </th> -->
	 <th align="left" class="header" >Destination </th>
	 <th align="left" width="20%" class="header" >Address </th>
	 <th align="left" class="header" >Cruise Website</th>
...
$rs=GetRecordList($select,_CRUISE_COMPANY_,$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
$crrs = GetPageRecord('name','countryMaster','id="'.$resultlists['countryId'].'"');
$crData = mysqli_fetch_assoc($crrs);

$crData['name'];

 $strs = GetPageRecord('name','stateMaster','id="'.$resultlists['stateId'].'"');
 $stData = mysqli_fetch_assoc($strs);

 $stData['name'];

 $ctrs = GetPageRecord('name','cityMaster','id="'.$resultlists['cityId'].'"');
 $ctData = mysqli_fetch_assoc($ctrs);
...
?>
  <tr>
    ...
    <td width="20%" align="left"><div  ><?php if($resultlists['address']!=''){ echo $resultlists['address'].' '.$ctData['name'].' '.$stData['name'].' '.$crData['name'].' '.$resultlists['pinCode']; } ?></div>   </td>
    <td width="10%" align="left"><div  ><?php echo $resultlists['website']; ?></div>   </td>
	
    <td width="8%" align="left"><?php if($resultlists['selfSupplier']==1){?><div ><?php echo 'Yes'; ?></div><?php } else { ?><div><?php echo 'No';?></div>     <?php } ?>   </td>
	
    <td width="8%" align="left"><div><strong>Name :&nbsp;</strong><?php echo $resultlists['contactPerson']; ?><br>
