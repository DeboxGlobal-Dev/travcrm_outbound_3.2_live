...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable"> 
   	<thead> 
		...
		<!--	<th width="12%" align="left" class="header" >&nbsp;</th>-->
			<th width="14%" align="left" class="header" >Photo </th>
  ...
  <tbody>
    ...
    if($startDate!='' && $endDate!=''){
    	  ...
    	  $endDate = date('Y-m-d', strtotime( $endDate ));
    	   $strWhere.=' and id in (select entranceId from hoteloperationRestriction where ( startDate BETWEEN "'.date('Y-m-d',strtotime($startDate)).'" and "'.date('Y-m-d',strtotime($endDate)).'" or  "'.date('Y-m-d',strtotime($endDate)).'" BETWEEN startDate and endDate ) and entranceId > 0 ) ';
    }
  ...
  <tr>
    <td><?=$no?></td>
    <td align="left"><?php if($resultlists['entranceImage']!=''){ ?><img src="packageimages/<?php echo $resultlists['entranceImage']; ?>" width="75" height="58" /><?php }else{ ?> <div style="width: 50px;height: 30px;background-color: grey;"></div> <?php } ?></td>
    ...
    <td align="left"><a href="showpage.crm?module=entrancerestrictions&entranceId=<?php echo encode($resultlists['id']); ?>"><?php echo $restrictionList['reason']?> (<?php echo $restrictionNm; ?>)</a></td>
	<td align="center"><a  href="#" onclick="masters_alertspopupopen('action=entrancerestriction&module=<?php echo $_REQUEST['module'];?>&id=<?php echo $resultlists['id']; ?>','600px','auto');"><input name="addnewuserbtn" type="button" class="bluembutton" value="+&nbsp;Operation&nbsp;Restriction"/></a></td>
  </tr> 
