...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=weekend','600px','auto');" />
  <?php } ?>
  </div>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add <?php echo "Weekend Name" ;////$pageName; ?>" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> <?php } ?>
      </tr>
...
<table border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
      ...
      <th align="left" class="header" >Weekend Name </th>
      <th align="left" class="header" >Weekend Days </th>
...
// $where='where 1 and deletestatus=0 order by id desc'; 
// $page=$_GET['page'];
...
$where='where 1 '.$wheresearch.''.$wheresearch2.' and deletestatus=0 order by name asc'; 

$page=$_GET['page'];
 
$targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 

$rs=GetRecordList($select,_WEEKEND_MASTER_,$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
  <tr>
    ...
    <!--<td align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div>   </td>-->
     <td width="13%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div>   </td>

    <td align="left"><?php  
	
	$weekenddays = explode(',', $resultlists['weekendDays']);

//print_r($weekenddays);
$days = $days224 = '';
  foreach ($weekenddays as  $value) {
 ...
 ?> </td>
	...
	<!--<td width="15%" align="left"><?php if($resultlists['status']==1){?><div style="border: 1px solid #809b7b; width: fit-content; padding: 5px 15px; background-color: #809b7b; color: #fff; border-radius: 3px;"><?php echo 'Active';?></div><?php } else { ?><div style="border: 1px solid #809b7b; width: fit-content; padding: 5px 8px; background-color: red; color: #fff; border-radius: 3px;"><?php echo 'In Active';?></div><?php }  ?>-->
	<!--</td>-->
