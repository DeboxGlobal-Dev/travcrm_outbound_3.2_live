...
<table width="100%" border="0" cellpadding="0" cellspacing="0"  >
  ...
  <tr>
    ...
    <td><div class="headingm" style="margin-left:5px;"><span id="topheadingmain"><?php if($_REQUEST['dashboard']!=''){?><a href="http://travcrm.in/"><img src="images/backicon.png" width="20" style=" cursor:pointer;" /></a><?php } ?> &nbsp;&nbsp;&nbsp;Series Listing</span>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0" width="100%" align="right">

      <tr>
 	
    <td width=60%"  align="right"><input name="searchField" type="text"  class="topsearchfiledmain" id="searchField"  value="<?php echo $searchField; ?>" placeholder="Query Id" onkeyup="numericFilter(this);"/></td>
...
$where2=' ( '.$wheresearchassign.'  or assignTo = '.$_SESSION['userid'].' or addedBy = '.$_SESSION['userid'].') and  assignTo!=""  group by assignTo order by assignTo asc';
...
?>
          ...
          <select name="queryshow" id="queryshow" class="topsearchfiledmainselect" style="width:160px; " onchange="changequerytstatus();" >
			...
			<option value="3" <?php if($_REQUEST['searchdate']==date('Y-m-d')){ ?> selected="selected"<?php } ?>>Today's Queries</option>

			<option value="4" <?php if(date('Y-m-d',strtotime("-1 days"))==$_REQUEST['searchdate']){ ?> selected="selected"<?php } ?>>Yesterday's Queries</option>

			<option value="5" <?php if('1'==$_REQUEST['thismonth'] && $_REQUEST['querystatus']!='3'){ ?> selected="selected"<?php } ?>><?php echo date('F'); ?>'s Queries</option>
...
?>
  ...
  <?php } ?>
        ...
        <td width="5%"><input name="module" id="module" type="hidden" value="<?php echo $_REQUEST['module']; ?>" /><input type="submit" name="Submit" value="Search" class="searchbtnmain" /></td>
 
	<!--//onclick="add();" setupbox('showpage.crm?module=query&add=yes');-->
        <?php if($addpermission==1){ ?><td width="5%"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add New" onclick="add();" style="margin-left: 5px !important;" /></td> <?php } ?>

		<td style="display: none;"><a href="showpage.crm?module=crmrates" class="bluembutton" style="margin: 0px; display:block;background-color: #9466be !important; border: 1px solid #9466be !important; font-weight:100;margin-left: 5px !important;">View&nbsp;Rates</a></td>
...
<table width="100%" border="0" cellpadding="5" cellspacing="0" style="display:none">
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=10 and moduleType=2  and deletestatus=0  '); ?></div>
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=2 and moduleType=2  and deletestatus=0 '); ?></div>
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=6  and moduleType=2 and deletestatus=0 '); ?></div>
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=7  and moduleType=2 and deletestatus=0 '); ?></div>
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=20  and moduleType=2 and deletestatus=0 '); ?></div>
...
<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=4  and moduleType=2 and deletestatus=0 '); ?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"  id="mainsectiontable" class="table table-striped table-bordered">
   ...
   <thead>
    ...
    <th align="left" class="header" >Series&nbsp;Code</th>

 	<th align="left" class="header">Series&nbsp;Name</th>

    <th align="left" class="header">Client&nbsp;Type</th>

    <th align="left" class="header">Client Name</th>

    <th align="center" class="header">Series&nbsp;Date </th>
    ...
    <th align="left" class="header" >Enquiry&nbsp;Type</th>
...
$where=' where '.$wheresearch.' '.$daterangeQuery.' '.$whereFromDate.' '.$assignto.' '.$mainwhere.' '.$destination.' '.$priority.' '.$querystatus.' '.$queryDateDash.' '.$moduleTypes.' and deletestatus=0 ORDER BY queryOrder DESC, dateAdded DESC ';
...
$where=' where '.$wheresearch.' '.$whereFromDate.' '.$daterangeQuery.' '.$assignto.' '.$mainwhere.' '.$destination.' '.$priority.' '.$querystatus.' '.$queryDateDash.' '.$moduleTypes.' and queryDate="'.$_REQUEST['searchdate'].'"  and deletestatus=0 ORDER BY queryOrder DESC, dateAdded DESC ';
...
$where=' where '.$wheresearch.' '.$whereFromDate.' '.$daterangeQuery.' '.$assignto.' '.$mainwhere.' '.$destination.' '.$priority.' '.$querystatus.' '.$queryDateDash.' '.$moduleTypes.' and queryDate>="'.date('Y-m').'-1" and queryDate<="'.date('Y-m-t').'"  and deletestatus=0 ORDER BY queryOrder DESC, dateAdded DESC ';
...
$where=' where '.$wheresearch.' '.$whereFromDate.' '.$daterangeQuery.' '.$assignto.' '.$mainwhere.' '.$destination.' '.$priority.' '.$querystatus.' '.$queryDateDash.' '.$moduleTypes.' and ( toDate>="'.date('Y-m-d').'" or dayWise=2 )  and deletestatus=0 ORDER BY queryOrder DESC, dateAdded DESC ';
...
/////echo $where;
$targetpage=$fullurl.'showpage.crm?module=query&records='.$limit.'&searchField='.$searchField.'&assignto='.$_GET['assignto'].'&queryshow='.$_REQUEST['queryshow'].'&searchdate='.$_REQUEST['searchdate'].'&thismonth='.$_REQUEST['thismonth'].'&';
...
setupbox('showpage.crm?module=query&view=yes&id='+id+'');
    ...
    <td align="left"><div class="bluelink" style="position:relative; padding-right:10px; font-weight:500; color:#45b558 !important; " onclick="viewSeries('<?php echo encode($resultlists['id']); ?>');"><?php echo trim($resultlists['seriesCode']); ?> </div>   </td>
    ...
    <td align="left"><?php if($resultlists['leadPaxName']!=''){ echo showClientTypeUserName($resultlists['clientType'],$resultlists['companyId']).'<br>'.'<b>Lead Pax:</b>&nbsp;'.$resultlists['leadPaxName']; }else{ echo showClientTypeUserName($resultlists['clientType'],$resultlists['companyId']); } ?></td>
    ...
    <td align="left" ><?php if($resultlists['queryType']==2 && $resultlists['moduleType']==1){?> <div class=" <?php echo 'pack3'; ?>">FIT</div><?php }  if($resultlists['queryType']==1 && $resultlists['moduleType']==1){?> <div class=" <?php echo 'pack2'; ?>">GIT</div><?php } ?><?php if($resultlists['queryType']==3  && $resultlists['moduleType']==1){?> <div class=" <?php echo 'pack2'; ?>">BROCHURE</div><?php } ?><?php if($resultlists['moduleType']==2){?> <div class=" <?php echo 'pack2'; ?>">SERIES</div><?php } ?></td>
