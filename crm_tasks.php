...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr> 
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Delete" onclick="alertspopupopen('action=corportatedelete&name=Tasks','600px','auto');" />
	<?php } ?>
	</div>
...
$rs2=GetPageRecord($select2,_TASKS_MASTER_,$where2); 
...
<input name="action" type="hidden" value="tasksdelete" id="action" />
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable">

   <thead>

   <tr>
     <th align="left" valign="middle" class="header" >#&nbsp;</th>
...
$where='where '.$wheresearchassign.'  '.$assignto.' '.$mainwhere.' '.$destination.' '.$priority.' '.$querystatus.' '.$searchFieldcommonquery.' and deletestatus=0 order by dateAdded desc'; 
$page=$_GET['page'];
 
$targetpage=$fullurl.'showpage.crm?module=tasks&records='.$limit.'&searchField='.$searchField.'&assignto='.$_GET['assignto'].'&'; 
...
?>
  <tr>
    <td align="left" valign="middle" style="padding:0px;"><?php if($editpermission==1){ ?>
      <div style="width:30px;"><img src="images/editicon.png" class="editicon" onclick="edit('<?php echo encode($resultlists['id']); ?>');" /></div><?php }   ?></td>
    
    <td align="left"><div style="max-width:200px;" class="bluelink" onclick="view('<?php echo encode($resultlists['id']); ?>');"><?php echo clean($resultlists['subject']); ?></div></td>
    <td align="left"><?php if($resultlists['clientType']==10){ echo 'My Task'; } else { echo showClientTypeUserName($resultlists['clientType'],$resultlists['companyId']); } ?></td>
