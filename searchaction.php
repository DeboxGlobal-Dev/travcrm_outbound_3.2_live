...
if($_REQUEST['action']=="getTransferNameAction" && $_REQUEST['destinationId']!=''){
   ...
   if($_REQUEST['action']=="loadAppendMultiServices" && $_REQUEST['id']!='' && $_REQUEST['serviceType']!=''){
    ...
    if($_REQUEST['action']=='loadAppendMultiCategoryRates' && $_REQUEST['id']!='' && $_REQUEST['quotationType']!=''){
        ...
        ?>
            ...
            <table border="1" cellspacing="0" cellpadding="4" style="position: relative;" id="removeId<?php echo $id; ?>">
                <tr>
                    <td>
						<select name="serviceTypeID" id="serviceTypeID<?php echo $id; ?>" onchange="showAdditionalCost('<?php echo $id; ?>');"  class="packageCostDiv_selectbox">
					...
					<td>
						...
						<select name="supplierId" id="supplierId<?php echo $id; ?>" class="packageCostDiv_selectbox">
					...
					<td><input type="text" name="serviceName" id="serviceName<?php echo $id; ?>" value="" placeholder="Service Name" style="padding: 3px;"></td>
                    ...
                    <td align="left"  >
						<select name="markupType" id="markupType<?php echo $id; ?>" class="gridfield1 validate" style="padding: 6px !important;width: 70px !important;">
					...
					<td align="left"  >
						<input type="text" name="markupValue" id="markupValue<?php echo $id; ?>" placeholder="Markup Value" class="gridfield1" displayname="Processing Fee" style="width: 60px !important;">
					</td> 
					<td>
						<select id="serviceTax<?php echo $id; ?>" name="serviceTax" class="gridfield" displayname="GST" autocomplete="off" style="padding: 6px !important;">
							<?php 
							$rs2="";
                    ...
                    <td>
						<select name="currencyId<?php echo $id; ?>" id="currencyId<?php echo $id; ?>" onchange="getROE(this.value,'currencyIdVal<?php echo $id; ?>');" class="packageCostDiv_selectbox" style="width:70px;">
                    ...
                    <td><input type="text" name="dayroe" id="currencyIdVal<?php echo $id; ?>" placeholder="ROE" value="<?php echo $dayroe; ?>" style="width: 74px;padding: 4px;"></td>
                    <td>
						<select name="costTypeId" id="costTypeId<?php echo $id; ?>" class="additionalCost<?php echo $id; ?> packageCostDiv_selectbox" style="display: none;">
							<option value="1">Per Person Cost</option>
							<option value="2">Group Cost</option>
						</select>
					</td>
                    </tr>
                    <tr class="costrow<?php echo $id; ?>">
                    ...
                    <div id="removeIcon<?php echo $id; ?>" style="position: absolute;right: 100px;"><i class="fa fa-trash" onclick="RemoveRate('<?php echo $id ?>');" aria-hidden="true" style="font-size: 20px;color: #01c901;cursor:pointer;color:red;"></i></div>
				</tr>
                <tr class="additionalrow<?php echo $id; ?>" style="display: none;">
					<td><input type="number" name="adultCost" id="adultCost<?php echo $id; ?>" placeholder="Adult Cost" value=""></td>
					<td><input type="number" name="adultPax" id="adultPax<?php echo $id; ?>" placeholder="Adult Pax" value=""></td>
					<td><input type="number" name="ChildCost" id="ChildCost<?php echo $id; ?>" placeholder="Child Cost" value=""></td>
					<td><input type="number" name="ChildPax" id="ChildPax<?php echo $id; ?>" placeholder="Child Pax" value=""></td>
					<td><input type="number" name="infantCost" id="infantCost<?php echo $id; ?>" placeholder="Infant Cost" value=""></td>
					<td><input type="number" name="infantPax" id="infantPax<?php echo $id; ?>" placeholder="Infant Pax" value=""></td>
				</tr>
    ...
    if($_REQUEST['action']=="loadCompletePackageWiseRates" && $_REQUEST['quotationId']!=''){
        $quotationId = $_REQUEST['quotationId'];
        $checkPackageRateQuery="";
		$checkPackageRateQuery=GetPageRecord('*','packageWiseRateMaster',' quotationId="'.$quotationId.'" order by id asc');
		 if(mysqli_num_rows($checkPackageRateQuery) > 0){
			while($getPackageRateData=mysqli_fetch_array($checkPackageRateQuery)){
	?>
			<table width="100%" border="1" cellspacing="0" cellpadding="4" style="margin-bottom: 5px;">
				<tr>
                    <th bgcolor="#ddd" >Supplier</th>
                    <th bgcolor="#ddd" >Service&nbsp;Type</th>
					<th bgcolor="#ddd" >Service&nbsp;Name</th>
					<th bgcolor="#ddd" >Markup</th>
					<th bgcolor="#ddd" >Tax&nbsp;Slab</th>
					<th bgcolor="#ddd" >Currency</th>
                    <?php if($getPackageRateData['serviceType']==2){ ?> 
					<th bgcolor="#ddd">Cost&nbsp;Type</th>
                    <?php } if($getPackageRateData['serviceType']==1){ ?>
                    ...
                    <?php } if($getPackageRateData['serviceType']==2){ 
                        if($getPackageRateData['costTypeId']==2){
                        ?>
                    ...
                    <?php } ?>
                    <th bgcolor="#ddd" align="center">#</th>
                    <th bgcolor="#ddd" align="center">#</th>
				</tr>
                <tr>
                    <td><?php echo getSupplierName($getPackageRateData['supplierId']) ?></td>
                    <td><?php if($getPackageRateData['serviceType']==1){ echo 'Land Package'; }else{ echo 'Additional'; } ?></td>
                    
                    <td><?php echo ($getPackageRateData['serviceName']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['markupValue']); echo ($getPackageRateData['markupType']==1)?'%':'Flat'; ?></td>
                    <td align="center"><?php echo ($getPackageRateData['serviceTax']); if($getPackageRateData['serviceTax']>0){ echo '%';} ?></td>
                    <td align="center"><?php echo getCurrencyName($getPackageRateData['currencyId']).'['.$getPackageRateData['ROE'].']'; ?></td>
                    <?php if($getPackageRateData['serviceType']==2){ ?> 
                    <td align="center"><?php if($getPackageRateData['costTypeId']==1){ echo 'Per Person Cost'; }else{ echo 'Group Cost'; } ?></td>
                    <?php } if($getPackageRateData['serviceType']==1){ ?>
                    <td align="center"><?php echo ($getPackageRateData['singleBasis']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['doubleBasis']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['tripleBasis']) ?></td>
                    <?php if(isRoomActive('quadroom')==true){ ?>
                    <td align="center"><?php echo ($getPackageRateData['quadBasis']) ?></td>
                    <?php } if(isRoomActive('sixbedroom')==true){ ?>
                    <td align="center"><?php echo ($getPackageRateData['sixBedBasis']) ?></td>
                    <?php } if(isRoomActive('eightbedroom')==true){ ?>
                    <td align="center"><?php echo ($getPackageRateData['eightBedBasis']) ?></td>
                    <?php } if(isRoomActive('tenbedroom')==true){ ?>
                    <td align="center"><?php echo ($getPackageRateData['tenBedBasis']) ?></td>
                    <?php } if(isRoomActive('teenbed')==true){ ?>
                    <td align="center"><?php echo ($getPackageRateData['teenBedBasis']) ?></td>
                    <?php } ?>
                    <td align="center"><?php echo ($getPackageRateData['extraBedABasis']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['childwithbedBasis']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['childwithoutbedBasis']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['infantBedBasis']) ?></td>
                    <?php } if($getPackageRateData['serviceType']==2){ 
                    if($getPackageRateData['costTypeId']==2){ ?> 
                    <td align="center"><?php echo ($getPackageRateData['groupCost']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['adultPax']) ?></td>
                    <?php } ?>
                    <td align="center"><?php echo ($getPackageRateData['adultCost']) ?></td>
                    <td align="center"><?php if($getPackageRateData['costTypeId']==1){ echo ($getPackageRateData['adultPax']); }else{ echo '0';} ?></td>
                    <td align="center"><?php echo ($getPackageRateData['ChildCost']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['ChildPax']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['infantCost']) ?></td>
                    <td align="center"><?php echo ($getPackageRateData['infantPax']) ?></td>
                    <?php } ?>
                    <td align="center"><div class="editbtnselect" id="selectthis" style="margin-top: 0px !important;cursor:pointer;" onclick="openinboundpop('action=editCompletePackageCost&editId=<?php echo $getPackageRateData['id'] ?>&quotationId=<?php echo $quotationId; ?>');" >Edit</div></td>

				    <td width="" align="center"><div><span style="color: #60ba3b; cursor: pointer; margin: 0px 5px;cursor:pointer;" ></span><span style="color: red;cursor: pointer; margin: 0px 5px;" onclick="if(confirm('Are you sure! you want to delete this Flight Rate?')){ deleteCompletePackageCost('<?php echo $getPackageRateData['id'] ?>','<?php echo $quotationId; ?>','deleteCompletePackageCost'); $('#addnl<?php echo $getPackageRateData['id'];?>').remove(); }" ><i class="fa fa-trash" aria-hidden="true"></i></span></div></td>
                </tr>
			</table>
            <style>
                .editbtnselect {
