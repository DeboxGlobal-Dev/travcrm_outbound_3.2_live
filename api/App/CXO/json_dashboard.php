...
// 	$thismonth_grossMargin = $thismonth_grossMarginData['sumTotalQueryCost']-$thismonth_grossMarginData['sumTotalQueryCostwithoutpercent'];
	...
	$thismonth_ConfirmQuerySql="select * from queryMaster where MONTH(queryConfirmingDate)=MONTH(now()) and YEAR(queryConfirmingDate)=YEAR(now()) and subject!='' and queryStatus='3' and deletestatus=0";
	...
	$$quotationGeneratedSql="select * from queryMaster where MONTH(quotationDate)=MONTH(now()) and YEAR(quotationDate)=YEAR(now()) and subject!='' and queryStatus='5' and deletestatus=0";
	$quotationGeneratedQuery=mysqli_query(db(),$$quotationGeneratedSql);
	$quotationGenerated=mysqli_num_rows($quotationGeneratedQuery);
// 	print_r($quotationGenerated);die();
...
$confirmQuery = countlisting($select,_QUERY_MASTER_,$where);
	...
	$json_result.= '{
		"salesincome" : "'.$grandTotalClient.'",
        "Todayquerypending" : "'.$queryPendingNM.'",
		"grossmargin" : "'.$grandTotalMargin.'",
        ...
        "thismonth_salesincome" : "'.$grandTotalClient.'",
        "thismonth_grossmargin" : "'.$grandTotalMargin.'",
