...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Meal-Plan','600px','auto');" />
	<?php } ?>
	</div>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add Meal Plan" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> <?php } ?>
...
<input name="action" id="action" type="hidden" value="deleterestaurantsmealplan" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
    ...
    <!--<label for="checkAll"><span></span>&nbsp;</label></th>-->
     ...
     <th align="left" class="header" >Meal Name </th>
...
$rs=GetRecordList($select,'restaurantsMealPlanMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
  <tr>
    ...
    <!--<td width="3%" align="center" valign="middle"><?php if($editpermission==156544){ ?><input type="checkbox" id="c<?php echo $no; ?>" name="check_list[]" class="chk"  value="<?php echo encode($resultlists['id']); ?>"/>-->
    <!--<label for="c<?php echo $no; ?>"><span></span>&nbsp;</label><?php } ?></td>-->
    <td width="5%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div>   </td>
	
	<td width="14%" align="left"><?php $select=''; 
$where2=''; 
$rs2='';  
...
<!--<td width="7%" align="left"><?php if($resultlists['status']==1){?><div style="border: 1px solid #809b7b; width: fit-content; padding: 5px 15px; background-color: #809b7b; color: #fff; border-radius: 3px;"><?php echo 'Active';?></div><?php } else { ?><div style="border: 1px solid #809b7b; width: fit-content; padding: 5px 8px; background-color: red; color: #fff; border-radius: 3px;"><?php echo 'In Active';?></div><?php }  ?></td>-->
	<td width="3%" align="left"><?php if($resultlists['status']==1){?><div style=" width: fit-content;  color: green; "><?php echo 'Active';?></div><?php } else { ?><div style=" width: fit-content; color: red; "><?php echo 'In Active';?></div><?php }  ?>
	</td>
	</tr> 
