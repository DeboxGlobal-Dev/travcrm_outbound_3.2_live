...
$email = $emailData['email'];
	   ...
	   $select1='*';
        ...
        $where2='masterId="'.$resListing['id'].'" and sectionType="contacts" ';
        $rs2=GetPageRecord('*','phoneMaster',$where2);
        $phoneData=mysqli_fetch_assoc($rs2);
        $phone = $phoneData['phoneNo'];
        $select3='*';
        $where3='masterId="'.$resListing['id'].'" and sectionType="contacts" order by id desc';
        $rs3=GetPageRecord($select,_EMAIL_MASTER_,$where3);
        $emailData=mysqli_fetch_array($rs3);
	    ...
	    $anniversaryDate=$resListing['anniversaryDate'];    
	    $dob=$resListing['birthDate'];    
	    $address_1=$resListing['address1']; 
	    
	    $new_date = date('m-d-Y', strtotime($date));
	    $new_anniversaryDate = date('m-d-Y', strtotime($anniversaryDate));
