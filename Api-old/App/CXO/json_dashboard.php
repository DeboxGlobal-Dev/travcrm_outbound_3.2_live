<?php
    ...
    //today saleincome and Gross Margin
	$todaySalesIncome=0;
	$todayGrossMargin=0;
	$salesIncomeSql= GetPageRecord('id,queryId',_QUOTATION_MASTER_,' status=1 and queryId in ( select id from queryMaster where queryConfirmingDate="'.date('Y-m-d').'" and queryStatus=3 )' );
	while($salesIncomeData=mysqli_fetch_array($salesIncomeSql)){
		$expenseAmount=0;
		$exrs = GetPageRecord('*','quotationExpensesMaster','queryId="'.$salesIncomeData['queryId'].'" and status=1 and queryId>0');
		while($expenseData = mysqli_fetch_assoc($exrs)){
			$expenseAmount = $expenseAmount + $expenseData['expenseAmount'];
		}

		$prm = GetPageRecord('*','paymentRequestMaster','quotationId="'.$salesIncomeData['id'].'"');
		$PRMData = mysqli_fetch_assoc($prm);
		$totalMarkupCost = $PRMData['totalMarkupCost']-$expenseAmount;
		$todaySalesIncome = $todaySalesIncome+$PRMData['totalClientCost'];
		$todayGrossMargin = $todayGrossMargin+$totalMarkupCost;
	}
	...
	$thismonthConfirmed=mysqli_num_rows($thismonth_Confirmed);
		...
		// UserWise Report
		$userWise_report = "SELECT * FROM userMaster WHERE `status`='1' ";
	...
	$json_result.= '{
		"salesincome" : "'.round($todaySalesIncome).'",
        "Todayquerypending" : "'.$queryPendingNM.'",
		"grossmargin" : "'.round($todayGrossMargin).'",
        ...
        "thismonth_salesincome" : "'.round($todaySalesIncome).'",
        "thismonth_grossmargin" : "'.round($todayGrossMargin).'",
