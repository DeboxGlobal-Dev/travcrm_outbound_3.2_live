...
// include "../inc.php";
	...
	if($_REQUEST['report'] == '1'){
		...
		$queryResultCount=mysqli_num_rows($Query);
			...
			$totalSalesSum=0;
			$totalMarginCost=0;
			$rs1 = GetPageRecord('id',_QUERY_MASTER_,'assignTo="'.$companyResult['id'].'" and queryStatus=3 and clientType!=2 '.$dateCon.'');
	
			while($queryId = mysqli_fetch_assoc($rs1)){
			    
			    $quotationquery= GetPageRecord('*',_QUOTATION_MASTER_,'queryId="'.$queryId['id'].'" and isPaymentRequest=1');
			
			    $quotationResult = mysqli_fetch_assoc($quotationquery);
				//Total Sales calculations
			    $totalSalesSum = $totalSalesSum + $quotationResult['totalQuotCost'];
			...
			// total margin calculations 
				...
				$rsp = GetPageRecord('*','paymentRequestMaster','quotationId="'.$quotationResult['id'].'"');
					...
					$MarginCost = $totalMarkupAMT-$expenseAmount;
					$totalMarginCost+=($MarginCost); 
			}
		...
		$json_result.= '{
			...
			"lost" : "'.$totalLost.'",
			"sales" : "'.round($totalSalesSum).'",
			"mat" : "'.$MatPersantage.'",
			"grossmargin " : "'.round($totalMarginCost).'",
			"totalpax" : "'.$pax.'",
