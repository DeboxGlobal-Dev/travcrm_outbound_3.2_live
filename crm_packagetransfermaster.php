...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
       ...
       <a name="addnewuserbtn" href="showpage.crm?module=dmcmaster" /><input type="button" name="Submit22" value="Back" class="whitembutton" > </a>    
    	...
    	<td><div class="headingm" style="margin-left:20px;"><span id="topheadingmain"><?php echo 'Transfer ';//$pageName; ?></span>
	...
	<?php } ?>
      ...
      <tr>
		...
		<td  ><a href="<?php echo $fullurl; ?>travrmimports/transfer-import-format.xls?t=<?php echo time(); ?>" class="bluembutton"  style="background-color: #1fc277 !important; border: 1px solid #1fc277 !important;"><i class="fa fa-download" aria-hidden="true"></i> Download Format</a></td>
                  ...
                  <td align="left" style="font-size: 14px;padding-left: 15px;">Rate Valid To</td></tr>
                    <tr>
                        ...
                        <td>
                          ...
                          <input type="text" id="toDate" name="toDate" value="<?php echo date('d-m-Y') ?>" class="topsearchfiledmainselect" style="width: 100px; text-align:center;">
                        </td>
                    ...
                    <tr>
                        <td colspan="6"><div id="downloadBtn" style="display:none;">
                            ...
                            $('#loadtransferData').load('downloadtransportData.php?action=searchTransferData&transferSupplier='+encodeURI(transferSupplier)+'&fromDate='+encodeURI(fromDate)+'&toDate='+encodeURI(toDate)+'&transferDestination='+encodeURI(transferDestination)+'&withRate='+encodeURI(withRate));
                            }
                            </script>
                        </div>
                    </td>
  ...
  <?php   $dirname =  'log_transfer/'; 
$images = scandir($dirname);
krsort($images);
foreach (array_slice($images, 0, 20) as $file) {
    if (substr($file, -4) == ".log" ) {
        ?>
		<a href="<?php echo $fullurl; ?>log_transfer/<?php echo $file; ?>" target="_blank"><?php echo $file; ?></a>
		<?php  
    }
}
 ?>
...
.dropdown:hover .dropbtn {background-color: #3e8e41;}
		...
		<td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add Transfer" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&trnsType=transfer','600px','auto');" /></td>      </tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <thead>
      ...
      <th align="left" class="header sorting" width="10%">Transfer Code</th>
      <th width="22%" align="left" class="header" >Transfer Name </th> 
      <th width="11%"  align="left" class="header" >Destinations</th>
      <th width="11%"  align="left" class="header" >Transfer Type</th>
...
$where='where 1 and transferCategory="transfer" and deletestatus=0 '.$wheresearch.' '.$wheresearch2.' order by id desc'; 
$page=$_GET['page'];
...
?>
  <tr>
    ...
    <td align="center"><?php  echo $serviceCode = makeServiceCode('TF',$resultlists['displayId']);  ?></td>
    
    <td align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>&trnsType=transfer','600px','auto');" ><?php echo $resultlists['transferName']; ?></div>   </td>
	...
	<td align="left"><?php 
    $rsMkt2=GetPageRecord('name','transferTypeMaster',' id="'.$resultlists['transferType'].'"');
    $transferTypeName='';
    if($resultlists['transferType'] == 0){ echo "All"; }elseif(mysqli_num_rows($rsMkt2)>0){
      $resMarkt2=mysqli_fetch_array($rsMkt2);
      echo $transferTypeName = $resMarkt2['name'];
    }
    ?></td>
	...
	<td align="center"><a href="showpage.crm?module=packagetransfermaster&supplier=1&transferid=<?php echo encode($resultlists['id']); ?>&view=yes"><input name="addnewuserbtn" type="button" class="bluembutton" value="+ Add Rate"/></a></td>
...
<form action="frm_action.crm" method="post" enctype="multipart/form-data" name="importfrm" id="importfrm"  target="actoinfrm" style="display:none;">
 <input name="importpackagetransfer" id="importpackagetransfer" type="hidden" value="Y" /> <input name="importpackagetransferModule" id="importpackagetransferModule" type="hidden" value="<?php echo clean($_GET['module']); ?>" />
