...
$officeId=clean($editresult['officeId']);
}
?>
...
<div class="addeditpagebox">
  ...
  <tr>
	...
	<div style="display: flex;">
		...
		<div class="griddiv">
			<label>
			<div class="gridlable" style="width: 132px;">Select Office <span class="redmind"></span></div>
			<select id="officeId" name="officeId" class="gridfield" displayname="office" autocomplete="off" >
			...
			$rs='';  
			$select='*';    
			$where='id!="'.$editresult['officeId'].'" and status=1  order by name asc';  
			$rs=GetPageRecord($select,'officeBranches',$where); 
			while($reportingmanagerlist=mysqli_fetch_array($rs)){   
			
			?>
			<option value="<?php echo strip($reportingmanagerlist['id']); ?>" <?php if($reportingmanagerlist['id']==$editresult['officeId']){ ?>selected="selected"<?php } ?>><?php echo $reportingmanagerlist['name']; ?></option>
			<?php }  ?>
			</select></label>
		</div>
	</div>
	...
	<input name="pin" type="number" class="gridfield validate" id="pin" value="<?php echo $editpin; ?>" maxlength="15" displayname="Password" /> 
	</label>
	</div>
	<div class="griddiv">
	<label>
...
?>" maxlength="60" displayname="Role Name" autocomplete="off" />
	...
	<div class="griddiv">
	<label>
	<div class="gridlable">Users Department <span class="redmind"></span></div>
	<select id="departmentId" name="departmentId" class="gridfield" displayname="Profile" autocomplete="off" >
...
$select='*';   
if($loginuseradmin==1){
$where='id="0" or userId="'.$loginusersuperParentId.'" order by id asc'; 
} else { 
...
?>
<option value="<?php echo strip($timeformat['id']); ?>" <?php if($timeformat['id']==$editdepartmentId){ ?>selected="selected"<?php } ?>><?php echo strip($timeformat['department']); ?></option>
<?php }} ?>
</select></label>
	</div>
		...
		<label>
			<div class="gridlable">User Type  </div>
				<select id="userType" name="userType" class="gridfield">	 
					...
					<option value="5" <?php if('5'==$edituserType){ ?>selected="selected"<?php } ?>>Finance</option> 
				</select>
		</label>
	</div>
...
<div class="rightfootersectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td>        </td>
        <td><input name="addnewuserbtn2" type="button" class="bluembutton submitbtn" id="addnewuserbtn2" value="Save" onclick="passwordvalidation(0);" /></td>
        <td><input type="button" name="Submit" value="Save and New" class="whitembutton submitbtn" onclick="passwordvalidation(1);"/></td>
...
function passwordvalidation(savenew) {
    ...
    if (oldpass != '' && pass != oldpass) {
        ...
        if (regex.exec(pass) == null) {
            ...
            formValidation('addeditfrm', 'submitbtn', savenew);
        }
    } else {
        formValidation('addeditfrm', 'submitbtn', savenew);
    }
