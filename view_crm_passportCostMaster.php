...
if($_REQUEST['passportId']!=''){  
	$aaaaaa=GetPageRecord('*','passportCostMaster',' id="'.decode($_REQUEST['passportId']).'"'); 
	$passportCostData=mysqli_fetch_array($aaaaaa);
}
?>
...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="93%" align="left"><?php echo 'Passport Cost' ?>:&nbsp;<?php echo $passportCostData['name']; ?></td>
  </tr>
...
<div id="addTriffRoom" style="display:nxone;">
	...
	<div class="addeditpagebox addtopaboxlist"> 
		<form action="frm_action.crm" method="post" enctype="multipart/form-data" name="addPassportPrice" target="actoinfrm"  id="addPassportPrice"> 
	...
	<div class="gridlable">Passport&nbsp;Name<span class="redmind"></span></div> 
		<input type="text" name="passportName" id="passportName" class="validate gridfield" value="<?php echo $passportCostData['name']; ?>">
	</label>  
	</div></td>
	...
	<div class="gridlable">Passport&nbsp;Type<span class="redmind"></span></div> 
		<select id="passportTypeId" name="passportTypeId" class="gridfield validate" displayname="Insurance Type" > 
				<?php   
				$rs='';   
				...
				?>
				<option value="<?php echo strip($passportType['id']); ?>" <?php if($passportType['id']==$passportCostData['passportType']){ ?> selected="selected" <?php }; ?> ><?php echo strip($passportType['name']); ?></option>
				<?php } ?>
				</select>
	</label>  
	</div></td>
		  	...
		  	<td width="100" align="left"><div class="griddiv" >
				...
				<?php   
				$rs='';   
				$rs=GetPageRecord('*',_SUPPLIERS_MASTER_,' deletestatus=0 and name!="" and status=1 and passportType=12  order by name asc'); 
	...
	<input name="toDate" type="text" id="toDate" class="gridfield calfieldicon validate" displayname="Rate Valid To" autocomplete="off" style="width: 100%;"/>
		  ...
		  <td width="100" align="left"  >
			...
			<td width="100" align="left" valign="middle"  ><input type="button" name="Submit" value="   Save   " class="bluembutton"  onclick="formValidation('addPassportPrice','saveflight','0');"> 
			  <input name="action" type="hidden" id="action" value="addPassportRate">
			  <input name="serviceid" type="hidden" id="serviceid" value="<?php echo $passportCostData['id']; ?>"></td>
			</tr> 
		</tbody></table>
		</form>
	</div>
...
<div id="loadPassportrate"></div> 

<script>   
function funloadPassportRate(){ 
$('#loadPassportrate').load('loadPassportRate.php?serviceid=<?php echo decode($_REQUEST['passportId']); ?>'); 
} 
