...
if($_REQUEST['leadId']!=''){
	...
	// get data for lead management B2C case 
	$leadId1 = decode($_REQUEST['leadId']);
	$select11='*';  
	$where11='id="'.$leadId1.'" '; 
	$rs11=GetPageRecord($select11,'leadManageMaster',$where11); 
	$resultlists2=mysqli_fetch_array($rs11);
	$editsubject=clean($resultlists2['subject']);
	$editguest1=clean($resultlists2['guest1']);
	$additionalInfo=clean($resultlists2['additionalInfo']);


	$editadult=clean($resultlists2['adult']);
	$editchild=clean($resultlists2['Child']);
	$editinfant=clean($resultlists2['Infant']);
	$editsglRoom=clean($resultlists2['sglRoom']);
	$editdblRoom=clean($resultlists2['dblRoom']);
	$edittwinRoom=clean($resultlists2['twinRoom']);
	$edittplRoom=clean($resultlists2['tplRoom']);
	$editextraNoofBed=clean($resultlists2['extraNoofBed']);
	$editquadNoofRoom=clean($resultlists2['quadNoofRoom']);

	$editcwbRoom=clean($resultlists2['cwbRoom']);
	$editcnbRoom=clean($resultlists2['cnbRoom']);
	$editleadsource=clean($resultlists2['leadsource']);
	$editdescription=clean($resultlists2['description']);
	}
	if($clientType!=2){ 
	$select2='*';  
	...
	// get data for lead management agent agent case 
	$leadId1 = decode($_REQUEST['leadId']);
	$select11='*';  
	$where11='id="'.$leadId1.'" '; 
	$rs11=GetPageRecord($select11,'leadManageMaster',$where11); 
...
if($_REQUEST['id']!=''){
	...
	$editassignToMain3=clean($editresult['assignTo3']);
	...
	$editsglRoom=clean($editresult['sglRoom']);
	$editdblRoom=clean($editresult['dblRoom']);
	$edittwinRoom=clean($editresult['twinRoom']);
	$edittplRoom=clean($editresult['tplRoom']);
	$editextraNoofBed=clean($editresult['extraNoofBed']);
	$editquadNoofRoom=clean($editresult['quadNoofRoom']);

	$editcwbRoom=clean($editresult['cwbRoom']);
	$editcnbRoom=clean($editresult['cnbRoom']);
...
if(isset($_REQUEST['quotationId'])){
	...
	if(mysqli_num_rows($quotSql)>0 ){ 
		...
		$dayWise=clean($quotationData['dayWise']);
		...
		// Pending FD
		$pendQuotSql=GetPageRecord('sum(sglRoom) as sglRoom, sum(dblRoom) as dblRoom, sum(tplRoom) as tplRoom, sum(twinRoom) as twinRoom, sum(childwithNoofBed) as childwithNoofBed,  sum(adult) as adult,  sum(child) as child , count(id) as cnt',_QUOTATION_MASTER_,' FDID="'.$quotationData['id'].'" and status=0 '); 
		if(mysqli_num_rows($pendQuotSql)>0){
		$pendQuotationData=mysqli_fetch_array($pendQuotSql);
		$pendingPax = $pendQuotationData['adult']+$pendQuotationData['child'];
		$pendingRooms = $pendQuotationData['sglRoom']+$pendQuotationData['dblRoom']+$pendQuotationData['tplRoom']+$pendQuotationData['twinRoom']+$pendQuotationData['extraNoofBed']+$pendQuotationData['childwithNoofBed']+$pendQuotationData['childwithoutNoofBed'];
		$rmadultPend = round($pendQuotationData['adult']-$quotationData['adult']);
		$rmchildPend = round($pendQuotationData['child']-$quotationData['child']);
		$rmsglRoomPend = round($pendQuotationData['sglRoom']-$quotationData['sglRoom']);
		$rmdblRoomPend = round($pendQuotationData['dblRoom']-$quotationData['dblRoom']);
		$rmtplRoomPend = round($pendQuotationData['tplRoom']-$quotationData['tplRoom']);
		$rmtwinRoomPend = round($pendQuotationData['twinRoom']-$quotationData['twinRoom']);
		$rmextraNoofBedPend = round($pendQuotationData['extraNoofBed']-$quotationData['extraNoofBed']);
		$rmcwbRoomPend = round($pendQuotationData['childwithNoofBed']-$quotationData['childwithNoofBed']);
		$rmcnbRoomPend = round($pendQuotationData['childwithoutNoofBed']-$quotationData['childwithoutNoofBed']);
		}
		//Confirmed FD
		$confQuotSql=GetPageRecord('sum(sglRoom) as sglRoom, sum(dblRoom) as dblRoom, sum(tplRoom) as tplRoom, sum(twinRoom) as twinRoom, sum(childwithNoofBed) as childwithNoofBed,  sum(adult) as adult,  sum(child) as child , count(id) as cnt',_QUOTATION_MASTER_,' FDID="'.$quotationData['id'].'" and status=1 '); 
		$confQuotationData=mysqli_fetch_array($confQuotSql);
		
		$rmadultN = round($quotationData['adult']-$confQuotationData['adult']);
		$rmchildN = round($quotationData['child']-$confQuotationData['child']);
		$rmsglRoomN = round($quotationData['sglRoom']-$confQuotationData['sglRoom']);
		$rmdblRoomN = round($quotationData['dblRoom']-$confQuotationData['dblRoom']);
		$rmtplRoomN = round($quotationData['tplRoom']-$confQuotationData['tplRoom']);
		$rmtwinRoomN = round($quotationData['twinRoom']-$confQuotationData['twinRoom']);
		$rmextraNoofBedN = round($quotationData['extraNoofBed']-$confQuotationData['extraNoofBed']);
		$rmcwbRoomN = round($quotationData['childwithNoofBed']-$confQuotationData['childwithNoofBed']);
		$rmcnbRoomN = round($quotationData['childwithoutNoofBed']-$confQuotationData['childwithoutNoofBed']);

		$confirmedPax = $confQuotationData['adult']+$confQuotationData['child'];
		$confirmedRooms = $confQuotationData['sglRoom']+$confQuotationData['dblRoom']+$confQuotationData['tplRoom']+$confQuotationData['twinRoom']+$confQuotationData['extraNoofBed']+$confQuotationData['childwithNoofBed']+$confQuotationData['childwithoutNoofBed'];
		
		$totalFDPax = $quotationData['adult']+$quotationData['child'];
		$totalFDRooms = $quotationData['sglRoom']+$quotationData['dblRoom']+$quotationData['tplRoom']+$quotationData['twinRoom']+$quotationData['extraNoofBed']+$quotationData['childwithNoofBed']+$quotationData['childwithoutNoofBed'];

		if($queryType==3){
			$editadult = $quotationData['adult'];
			 $editchild = $quotationData['child'];
			 $editsglRoom=clean($quotationData['sglRoom']);
			 $editdblRoom=clean($quotationData['dblRoom']);
			 $edittwinRoom=clean($quotationData['twinRoom']);
			 $edittplRoom=clean($quotationData['tplRoom']);
			 $editextraNoofBed=clean($quotationData['extraNoofBed']);
			 $editquadNoofRoom=clean($quotationData['quadNoofRoom']);
		 
			 $editcwbRoom=clean($quotationData['cwbRoom']);
			 $editcnbRoom=clean($quotationData['cnbRoom']);

			 $availblePax = $totalFDPax-($confirmedPax+$pendingPax);
			 $availbleRooms = $totalFDRooms-($confirmedRooms+$pendingRooms);

			$totalFDPaxCount = '<span style="font-weight:500;padding: 0px 0 10px 0px;display: block;"> Total Pax '.$totalFDPax.' Confirmed '.$confirmedPax.' Pending '.$pendingPax.' Available '.$availblePax.'</span>';

			$totalFDRoomCount = '<span style="font-weight:500;padding: 0px 0 10px 0px;display: block;"> Total Rooms '.$totalFDRooms.' Confirmed '.$confirmedRooms.' Pending '.$pendingRooms.' Available '.$availbleRooms.'</span>';
		}
		...
		if($confQuotationData['cnt'] > 0 && $queryType==3){  
			...
			$rmadult = '<span style="line-height: 2; font-size:13px;color:#'.$colorsgl.';">'.round($quotationData['adult']).' Available </span>';
			$rmchild = '<span style="line-height: 2; font-size:13px;color:#'.$colorsgl.';">'.round($quotationData['child']).' Available </span>';
			$rmsglRoom = '<span style="line-height: 2; font-size:13px;color:#'.$colorsgl.';">'.round($quotationData['sglRoom']).' Available </span>';
			$rmdblRoom = '<span style="line-height: 2; font-size:13px;color:#'.$colordbl.';">'.round($quotationData['dblRoom']).' Available </span>';
			$rmtplRoom = '<span style="line-height: 2; font-size:13px;color:#'.$colortpl.';">'.round($quotationData['tplRoom']).' Available </span>';
			$rmextraNoofBed= '<span style="line-height: 2; font-size:13px;color:#'.$colorEB.';">'.round($quotationData['extraNoofBed']).' Available </span>';
			$rmtwinRoom= '<span style="line-height: 2; font-size:13px;color:#'.$colortwin.';">'.round($quotationData['twinRoom']).' Available </span>';
			$rmcwbRoom= '<span style="line-height: 2; font-size:13px;color:#'.$colorcwb.';">'.round($quotationData['cwbRoom']).' Available </span>';
			$rmcnbRoom= '<span style="line-height: 2; font-size:13px;color:#'.$colorcnb.';">'.round($quotationData['cnbRoom']).' Available </span>';
		}
	}
} 
...
<div class="rightsectionheader" style="margin-top: 40px;padding: 20px 0px 5px">
...
<div id="pagelisterouter" style="padding-left:0px;margin-top: -50px;">
...
<div style="display: flex;width: 100%;">
	<div style="text-align: left;width: 80%;">
		...
		<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Contact Information | </span> <span id="bussinestypechoice" style="margin-left: 12px;position: relative;font-size: 13px;font-weight: bold;color: #6d17bc;top: -3px;"></span>
	</div>
	<div style="text-align: right; width: 20%;">
		<span>
			<img class="BussinesSecDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="BussinesSec()">
			<img class="BussinesSecUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="BussinesSec()">
		</span>
	</div>
		...
		<?php
			...
			while($resListing1=mysqli_fetch_array($rs)){
				...
				if($clientType!='' && $clientType!='0'){
					...
					$clientDefault = $resListing1['setDefault']==1;
				}
			?>
			<div class="main-div-con" style="width:70%" >
				<div class="check_boxIndiv" style="display: flex; width:100%">

					<div style="width:50%;text-align: left;" >
						<?php echo ucfirst($resListing1['name']); ?>
					</div>
					<div style="width:50%;text-align: right;" class="check-box-title" >
					
						<input type="radio" id="clientType" name="clientType" style="display: block;"
						 <?php if($clientDefault){ echo "checked"; } ?>  value="<?php echo strip($resListing1['id']); ?>"  class="check_box_input" onclick="selectBusinessType();" >
...
<!--Ended bussines type select in any place  -->
	  ...
	  <tr>
		...
		?>
			...
			<?php if($profileeeDataaaaa['agentOption']==1){ ?>
				<div style="padding:10px; background-color:#7a96ff; color:#fff; position:absolute; right:1px; top:4px; cursor:pointer;" onclick="addclientfromquery('action=addAgentClienttoQuery&actionType=addserviceagentclient','800px');">+Add</div>
			<?php } ?>
	...
	<div class="agentsec-select" style="height: 85px;
    border: 1px solid #e0e0e0; border-radius: 5px;background: gray;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" id="banumber" style="border: 0px solid;display:<?php if($clientType!=''){ ?>block<?php } else { ?>none<?php } ?>; ">
 	  <tr style="position: relative;top: 20px;">

		<td width="48%"><div class="griddiv" style="width: 95%;border: 0px;"><label>

		<div class="gridlable" >

		
		  <c id="contactpersonnamespan" style="color: white!important;margin-left:5px;">Name - <input name="agentb2cname" type="text" class="gridfield" id="agentb2cname"  displayname="Contact Person" value="<?php echo $clientnemdisplay.$clientnemdisplayfrommail; ?>" style="border: 0px solid black;position: relative;top: -28px;left: 44px;background: none;color: white" readonly/>  </c>
		...
		<td width="48%"><div class="griddiv" style="border: none;"><label>

		<div class="gridlable" style="color: white!important;">Contact Number-
			
		</div>

		<input name="agentb2cnumber" type="text" class="gridfield" id="agentb2cnumber"  displayname="Phone/Mobile" value="<?php echo $getphone;?>" style="border: 0px solid black;position: relative;top: -28px;left: 98px;background: none;color: white" readonly />
	  ...
	  <tr style="position: relative;top: -20px;">
		<td colspan="2">
		<div class="griddiv" id="baemail" style="border: 0px;"><label>

		<div class="gridlable" style="font-size: 13px;color: white!important;margin-left:5px;">Email Address-

		<input name="agentb2cmail" type="email" class="gridfield" id="agentb2cmail"  displayname="Email"    value="<?php echo $getemail; ?>"  style="border: 0px solid black;position: relative;top: -30px;left: 95px;background: none;color: white" readonly/>
		<span class=""></span></div>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 10px;">
	  <tr>
		...
		<input name="guest1" type="text" class="gridfield"  id="guest1"  
		value="<?php echo $clientnemdisplay.$clientnemdisplayfrommail; ?>" maxlength="100" />
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" style=" ">
	  <tr>
		...
		<tr>
			...
			<textarea  name="additionalInfo" class="gridfield" id="additionalInfo" style="min-height: 60px;"> 
			

			
			<?php 
				if($additionalInfo!=''){
					echo stripslashes($additionalInfo);
				}else{
					echo stripslashes(strip_tags($editdescription));
				}
			?>
...
function BussinesSec() {

var x = document.getElementById("Bussines_Hide");
var upimg = document.getElementsByClassName("BussinesSecUp");
var downimg = document.getElementsByClassName("BussinesSecDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.BussinesSecUp').show();
	$('.BussinesSecDown').hide();
} else {
	...
	// $('.QuerySecUp').hide();
		...
		function selectBusinessType() {
			var queryTypechoice = $("input[type='radio'][name='clientType']:checked").val();
			// var queryTypechoice = document.getElementById("clientType").value;
			// alert(queryTypechoice);
			if(queryTypechoice==1){
				var queryName = 'Agent';
			}
			if(queryTypechoice==2){
				var queryName = 'B2C';
			}
			if(queryTypechoice==89){
				var queryName = ' Corporate';
			}
			
			document.getElementById("bussinestypechoice").innerHTML = queryName;
		}
		selectBusinessType();
	</script>
	...
	<div style="text-align: left;width: 50%;">
		<img style="font-size: 14px;position: relative;top: 0px;height: 20px;" src="images/question-dom.png"> 
		<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Query Type | </span> <span id="querytypechoice" style="margin-left: 12px;
		position: relative;
		font-size: 13px;
		font-weight: bold;
		color: #6d17bc;
	...
	<div style="text-align: right; width: 50%;">
		<span>
			<img class="QuerySecDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="QuerySec()">
			<img class="QuerySecUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="QuerySec()">
		</span>
	</div>
	...
	<div style=" border:1px #ccc solid; padding:10px;border-top:0px;display:block; margin-bottom: 20px;display:none;" id="QuerySec_Hide">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" >
		<tr>
					...
					<tr> 
							<td width="50%">
								<div class="griddiv">
									...
									<?php
									$rs='';
									$rs=GetPageRecord('*','moduleTypeMaster','status=1 and id in("1","3","4","5","13","14") order by id asc');
									...
									<div class="" style="width: 100%;">
										<div class="check_boxIndiv">
											...
											<input type="radio" name="queryType" id="queryType<?php echo $moduleTypeData['id']; ?>" onclick="gitcodefun('<?php echo strip($moduleTypeData['id']); ?>');selectQueryType();" class="check_box_input" value="<?php echo strip($moduleTypeData['id']); ?>" <?php if($moduleTypeData['id']==$queryType){ echo "checked"; }elseif($moduleTypeData['id']==1){ echo 'checked'; } ?> ></input>
										</div>
...
var x = document.getElementById("QuerySec_Hide");
var upimg = document.getElementsByClassName("QuerySecUp");
var downimg = document.getElementsByClassName("QuerySecDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.QuerySecUp').show();
	$('.QuerySecDown').hide();
...
function selectQueryType() {
	var queryTypechoice = $("input[type='radio'][name='queryType']:checked").val();
	// alert(queryType);
	if(queryTypechoice==1){
		var queryName = 'Query';
	}
	if(queryTypechoice==3){
		var queryName = 'Fixed Departure';
	}
	if(queryTypechoice==4){
		var queryName = 'Package';
	}
	if(queryTypechoice==5){
		var queryName = 'Duplicate Query';
	}
	if(queryTypechoice==13){
		var queryName = 'Multiple Services';
	}

	if(queryTypechoice==14){
		var queryName = 'Activity';
	}
	document.getElementById("querytypechoice").innerHTML = queryName;
}
</script>

<!-- Started Service Selection sec -->
	<div id="main-service-selection" style="margin-top: 10px;">
	<div style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;    border-top-right-radius: 5px;
    border-top-left-radius: 5px" onclick="$('#showmorefield1').toggle();">
...
<div style="display: flex;width: 100%;">
	...
	<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Value Added Service Selection</span>
	</div>
	<div style="text-align: right; width: 50%;">
	<span>
	<img class="ServiceSelectionDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="ServiceSelectionSec()">
	<img class="ServiceSelectionUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="ServiceSelectionSec()">
	</span>
	</div>
</div>
	...
	<div style=" border:1px #ccc solid; padding:10px;border-top:0px;display:block; margin-bottom: 20px;display:none;" id="ServiceSelectionSec_Hide">
	<table  width="100%" border="0" cellpadding="0" cellspacing="0">
...
<script>
function ServiceSelectionSec() {
	// ServiceSelectionUp,ServiceSelectionDown,ServiceSelectionSec,ServiceSelectionSec_Hide
var x = document.getElementById("ServiceSelectionSec_Hide");
var ServiceSelectionUp = document.getElementsByClassName("ServiceSelectionUp");
var ServiceSelectionDown = document.getElementsByClassName("ServiceSelectionDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.ServiceSelectionUp').show();
	$('.ServiceSelectionDown').hide();
} else {
	x.style.display = "none";
	$('.ServiceSelectionDown').hide();
	// $('.ServiceSelectionUp').show();
...
<div style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;    border-top-right-radius: 5px;
	...
	<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Travel Info | </span>
	<span id="Travelchoice" style="margin-left: 12px;
    position: relative;
    font-size: 13px;
    font-weight: bold;
    color: #6d17bc;
    top: -3px;"></span> <span style="color: hsl(33 95% 68% / 1);font-weight: 100; position: relative;
    top: -4px;"> |</span> 

	<span id="" style="margin-left: 3px;
    position: relative;
    font-size: 13px;
    font-weight: bold;
    color: #6d17bc;
    top: -3px;">Guest Type - <span id="paxchoice" style="margin-left: 3px;
    position: relative;
    font-size: 13px;
    font-weight: bold;
    color: #6d17bc;
	...
	<div style="text-align: right; width: 20%;">
	<span>
	<img class="TravelInformationDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="TravelInformationSec()">
		<img class="TravelInformationUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="TravelInformationSec()">
	</span>
	...
	<div style=" border:1px #ccc solid; padding:10px;border-top:0px;display:block; margin-bottom: 20px;display:none;" id="TravelInformation_Hide">



	
<!--Started bussines type select in any place  -->
<div class="gridlable" style="font-size: 13px;color: gray;">Travel&nbsp;Type</div>
	<div >
		<div  style="display: grid;grid-template-columns: auto auto; width:100%;">
		<?php 
			...
			<style>
				<?php 
					if($rescompanysetting['internationalQuery']==1 && $rescompanysetting['domesticQuery']==1){?>
					.check_boxIndiv{
						width:80%;	
					}
				<?php }else{?>
			...
			<div class="main-div-con" style="width: 100%;" >
				<div class="check_boxIndiv" style="display: flex;">
					...
					<div style="width:50%;text-align: right;" class="check-box-title" >
						...
						<input name="travelType" id="travelType" class="check_box_input" <?php if($travelType==1 || $travelType==''){ echo "checked"; }elseif($rescompanysetting['defaultQueryType']==1){ echo  "checked";  } ?> type="radio" value="1" onchange="selectTravelType()">
			...
			<?php if($rescompanysetting['domesticQuery']==1){ ?>
			<div class="main-div-con" style="width: 100%;margin-left: 0px;" >
				<div class="check_boxIndiv" style="display: flex;">
					...
					<div style="width:50%;text-align: right;" class="check-box-title" >
						...
						<input name="travelType" id="travelType" 
							class="check_box_input" type="radio" value="2" 
							<?php
							 if($travelType==2){ echo "checked"; }  elseif($rescompanysetting['defaultQueryType']==2){ echo  "checked";  } ?>
							 onchange="selectTravelType()"></input>
		...
		<div class="gridlable" style="font-size: 13px;color: gray;">Pax&nbsp;Type</div>
		<div  style="display: grid;grid-template-columns: auto auto; width:100%;">
			...
			<div class="main-div-con" style="width:85%" >
				<div class="check_boxIndiv" style="display: flex; width:100%">
					...
					<div style="width:50%;text-align: right;" class="check-box-title" >
					<input name="paxType" id="paxType" class="check_box_input"  type="radio" value="1" <?php if(1==$paxType || $paxType==''){ ?>checked="checked"<?php } ?> onchange="selectPaxType()" ></input>
					</div>
			...
			<div class="main-div-con" style="width: 76%; margin-left: 16px;" >
				<div class="check_boxIndiv" style="display: flex; width:100%">
					...
					<div style="width:50%;text-align: right;" class="check-box-title" >
					<input name="paxType" id="paxType"  class="check_box_input" type="radio" value="2" <?php if(2==$paxType){ ?>checked="checked"<?php } ?> onchange="selectPaxType()" checked="checked"></input>
					</div>
...
<!--ended  travel info top sec -->

<script>
selectTravelType();
function selectTravelType() {
	var queryTypechoice = $("input[type='radio'][name='travelType']:checked").val();
	// alert(queryType);
	if(queryTypechoice==1){
		var queryName = 'International';
	}
	if(queryTypechoice==2){
		var queryName = 'Domestic';
	}
	
	document.getElementById("Travelchoice").innerHTML = queryName;
}
</script>

<script>
selectPaxType();
function selectPaxType() {
	var queryTypechoice = $("input[type='radio'][name='paxType']:checked").val();
	// alert(queryType);
	if(queryTypechoice==2){
		var queryName = 'FIT';
	}
	...
	document.getElementById("paxchoice").innerHTML = queryName;
}
</script>



<div id="tranvel_info1">
<div style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;    border-top-right-radius: 5px;
    border-top-left-radius: 5px;margin-top: 20px;" onclick="$('#showmorefield1').toggle();">
	<img style="font-size: 14px;position: relative;top: 0px;height: 20px;" src="images/information-dom.png"> 
	<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Travel Date  </span>

</div>

	

	<div style=" border:1px #ccc solid; padding:10px;border-top:0px;display:block; margin-bottom: 0px;display:none1;" id="TravelDate_Hide">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <!-- day and date sec -->
		<td width="40%" style="display: none1;"><label><select name="dayWise" id="dayWise"  class="gridfield" onchange="changedatetr();" style="padding:10px; width:100%; border:1px solid #ccc; box-sizing:border-box; margin-bottom:10px;">
...
<input name="toDate2" type="text" id="toDate2"  class="gridfield calfieldicon" displayname="To Travel Date" autocomplete="off" value="<?php if($edittoDate!='1970-01-01' && $edittoDate != '' ){ echo date('d-m-Y',strtotime($edittoDate)); }else{ echo date('d-m-Y'); } ?>" readonly="readonly" style="position: relative; top: auto; right: auto; bottom: auto; left: auto;">
	...
	<div class="gridlable" id="TotalDayNight">Total Nights <span class="redmind"></span></div>
	<div class="gridlable" id="TotalDayNightDays" style="display:none;">Days <span class="redmind"></span></div>
	...
	<td width="17%" align="left" valign="top"><a href="javascript:void(0)" style="font-size: 12px;background-color: #7a96ff;color: #fff !important;padding: 11px 8px;margin-right: 0px;margin-top: 20px;display: block;text-align: center;margin-left: 1px;" onclick="generateQueryDay_function();">+ Add</a></td>
	...
	<div style="border: 0px #e0e0e0 solid; padding:0px; text-align:right; margin-bottom:10px;" id="adddatedistinationdiv"></div>
<script type="text/javascript">
...
<script>
function TravelInformationSec() {
	// TravelInformationUp,TravelInformationDown,TravelInformationSec,TravelInformation_Hide
var x = document.getElementById("TravelInformation_Hide");
var TravelInformationUp = document.getElementsByClassName("TravelInformationUp");
var TravelInformationDown = document.getElementsByClassName("TravelInformationDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.TravelInformationUp').show();
	$('.TravelInformationDown').hide();
} else {
	x.style.display = "none";
	$('.TravelInformationDown').hide();
	// $('.QuerySecUp').hide();
	...
	<div id="pax_detail_box" style="margin-top: -20px;">
	<div style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;    border-top-right-radius: 5px;
    border-top-left-radius: 5px" onclick="$('#showmorefield1').toggle();">
...
<div style="display: flex;width: 100%;">
	...
	<div style="text-align: right; width: 50%;">
	<span>
	<img class="QuotaItinInfoDown errow-drop" style="display:none;" src="images/down-arrow-30.png"  onclick="QuotaItinInfoSec()">
	<img class="QuotaItinInfoUp errow-drop" style="" src="images/collapse-arrow.png"  onclick="QuotaItinInfoSec()">
	</span>
	</div>
</div>
	...
	<div style=" border:1px #ccc solid; padding:10px;border-top:0px;display:block;display:block;" id="QuotaItinInfo_Hide">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="color: gray;">
		<tr>
			<td colspan="4" align="left" valign="top">
			<table style="display: none1;" width="100%" border="0" class="griddiv" cellpadding="0" cellspacing="0" id="paxCountId">
				<!-- <div><p>Pax Information</p></div> -->

				<div style="display:flex;">
					<div><p>Pax Information</p></div>
					<div style="display: inline-block;width: 75px;font-size: 15px;font-weight: 500;padding: 5px;color: #fff;background-color: green;height: 20px;margin-top: 6px;margin-left: 10px;">Total Pax:- </div>
					<input id="totalpax" readonly name="totalpax" value="0" style="display: inline-block;font-size: 15px;font-weight: 500;padding: 5px;border:none !important; width:60px;">
				</div>
				...
				<input name="adult" type="text" oninput="getPaxCount(1)" class="gridfield validate" style="width: 94%;" onKeyUp="numericFilter(this);" id="adult" displayname="Adult" value="<?php echo $editadult; ?>" maxlength="3" />


				<!-- <input style="width: 94%;" name="sglRoom" type="text" oninput="getRoomCount(1)" class="gridfield mb5 numeric" id="sglRoom" value="<?php if($leadId1!=''){echo $editsglRoom;}else{ echo $editresult['sglRoom'];} ?>" maxlength="3"> -->


				</label><?php if($queryType ==3){ echo $rmadult; } ?></td>
				...
				<input name="child" type="text" oninput="getPaxCount(2)" style="width: 94%;" class="gridfield" id="child" onKeyUp="numericFilter(this);showcwbroom();showChildAge();showCNBCWBRoom();" displayname="Child" value="<?php echo $editchild; ?>" maxlength="3" />
				...
				<input name="infant" type="text" oninput="getPaxCount(3)" style="width: 94%;" class="gridfield" id="infant" onKeyUp="numericFilter(this);" displayname="Infant" value="<?php if($leadId1!=''){echo $editinfant;}else{echo $infant;} ?>" maxlength="3" />
				...
				<div><?php echo $totalFDPaxCount; ?></div>
				<script>
	...
	<table  width="100%" border="0" class="griddiv" cellpadding="0" cellspacing="0" id="roomTypeId" >
		...
		<div style="display:flex;" id="roomInformation">
			<div><p>Room Information</p></div>
			<div style="display: inline-block;width: 104px;font-size: 15px;font-weight: 500;padding: 5px;color: #fff;background-color: green;height: 20px;margin-top: 6px;margin-left: 10px;">Total Rooms:- </div>
			<input id="totalrooms" readonly name="totalrooms" value="0" style="display: inline-block;font-size: 15px;font-weight: 500;padding: 5px;border:none !important; width:60px;">
		</div>


		<tr style="width:100%">
			<td align="left" colspan="2" valign="top" style="width:20%">Single</td>
			<td align="left" colspan="2" valign="top"  style="width:20%">Double</td>
			<td align="left" colspan="2" valign="top"  style="width:20%">TWIN</td>
			<td align="left" colspan="2" valign="top"  style="width:20%">Triple</td>
			<td align="left" colspan="2" valign="top"  style="width:20%">ExtraBed(A)</td>
		</tr>
		

		<tr>

			<td align="left" colspan="2" valign="top"><input style="width: 94%;" name="sglRoom" type="text" oninput="getRoomCount(1)" class="gridfield mb5 numeric" id="sglRoom" value="<?php echo $editsglRoom; ?>" maxlength="3">
				<?php if($queryType ==3){ echo $rmsglRoom; } ?>
			</td>

			<td align="left" colspan="2" valign="top"><input style="width: 94%;" name="dblRoom" type="text" oninput="getRoomCount(2)" class="gridfield mb5 numeric" id="dblRoom" value="<?php echo $editdblRoom; ?>" maxlength="3">
				<?php if($queryType ==3){ echo $rmdblRoom; } ?>
			</td>
			<td align="left" colspan="2" valign="top"><input style="width: 94%;" name="twinRoom" type="text" oninput="getRoomCount(3)" class="gridfield mb5 numeric" id="twinRoom" value="<?php echo $edittwinRoom; ?>" maxlength="3">
				<?php if($queryType ==3){ echo $rmtwinRoom; } ?>
			</td>
			<td align="left" colspan="2" valign="top"><input style="width: 94%;" name="tplRoom" type="text" oninput="getRoomCount(4)" class="gridfield mb5 numeric" id="tplRoom" value="<?php echo $edittplRoom; ?>" maxlength="3">
				<?php if($queryType ==3){ echo $rmtplRoom; } ?>
			</td>
			<td align="left" colspan="2" valign="top"><input  style="width: 94%;" name="extraNoofBed" type="text" class="gridfield mb5 numeric" id="extraNoofBed" value="<?php echo $editextraNoofBed; ?>" maxlength="3">
				<?php if($queryType ==3){ echo $rmextraNoofBed; } ?>
			</td>	
		</tr>

		<!-- Started new show child with bad and child no bad -->
		<tr  id="shownotcnbcwb1" style="display:none; width:200%;" >
			<td align="left" colspan="2" valign="top" style="width:20%">CWBed</td>
			<td align="left" colspan="2" valign="top" style="width:20%">CNBed </td>
		</tr>
		<tr id="shownotcnbcwb2" style="display:none;width:200%;">
			<td align="left" colspan="2" valign="top" style="width:20%">
			<!-- onkeyup="balanceCWB(this.value)" -->
				<div class="showcwbroom">
				<input name="cwbRoom" style="width: 94%;" type="text" class="gridfield mb5 numeric" id="cwbRoom" value="<?php echo $editcwbRoom; ?>" maxlength="3" >
				<?php if($queryType ==3){ echo $rmcwbRoom; } ?>
			</div>
			</td>
			<td align="left" colspan="2" valign="top" style="width:20%">
				<div class="showcwbroom">
				<input name="cnbRoom" type="text" style="width: 94%;" class="gridfield mb5 numeric" id="cnbRoom" value="<?php echo $editcnbRoom; ?>" maxlength="3" >
				<!-- onkeyup="balanceCNB(this.value)" -->
				<?php if($queryType ==3){ echo $rmcnbRoom; } ?>
			</div>
			</td>
		</tr>
		<!-- Ended new show child with bad and child no bad -->
		<tr>
		...
		<tr>
		    <?php if(isRoomActive('quadroom')==true){ ?>  
			 <td align="left" colspan="2" valign="top">
				<input name="quadNoofRoom" type="text" style="width: 94%;" class="gridfield mb5 numeric" oninput="getRoomCount(6)" id="quadNoofRoom" value="<?php if($leadId1!=''){echo $editquadNoofRoom;}else{echo $editresult['quadNoofRoom'];} ?>" maxlength="3">
			</td>
			<?php } ?>
			<?php if(isRoomActive('sixbedroom')==true){ ?> 
			<td align="left" colspan="2" valign="top">
	...
	<div><?php echo $totalFDRoomCount; ?></div>
	</div>
	<script>
...
<table width="100%" border="0" cellpadding="4" cellspacing="0"  id="childfielddiv" style="display:none; ">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		<td width="55%" style="color:white;border-radius: 5px;">

		<!-- <div class="griddiv  validate" style="width: 100%;border-bottom: 0px #eee solid;margin-bottom: 7px;"> 
			<label> 
				<div class="gridlable " style="width:100%;display:flex;position: relative;top: 9px;left: 3px;">
				<span class="" style="width: 61%;background: none;color: white;">Sales Person</span>

				<input style="border: 0px;margin-top: -8px;background: none;color:white;" type="text" name="salesassignTo" id="salesassignTo" class="gridfield" value="<?php echo getUserName($salesPersonId); ?>" readonly="" disabled/></div> 
		...
		<div class="griddiv validate " style="width: 95%;background: white;"><img src="images/add-user-dom.png" onclick="function_assignTo113();" style="position: absolute;right: 0px;cursor: pointer;right: 20px;
			top: 25px;border-radius: 5px;width: 20px;"  />
		<label>
		<div class="gridlable " style="width:100%;">Sales Person<span class="redmind"></span></div>
		<div id="selectOpsPerson">
		<input style="" name="salesassignTo" type="text" class="gridfield validate " id="salesassignTo" value="<?php echo getUserName($salesPersonId); ?>" readonly="true" displayname="Sales&nbsp;Person" autocomplete="off" onclick="function_assignTo113();" />
		<input name="salesPersonId" type="hidden" id="salesPersonId" value="<?php echo encode($salesPersonId); ?>" /></div>
		</label>
		<script type="text/javascript">
		function function_assignTo113(){
			var lang = $('#language').val();
			alertspopupopen('action=selectParentSale&userType=0','600px','auto');
		}
		</script>
		</div>
	  ...
	  <tr>
		<td width="48%">
		<div class="griddiv  " style="width: 96%;"><img src="images/add-user-dom.png" onclick="function_assignTo();" style="position:absolute; right:0px; cursor:pointer; right:4px; top:26px;"  />
		<label>
		<div class="gridlable " style="width:100%;">Operation Person<span class=" "></span></div> 
		<div id="selectOpsPerson"> 
			<input name="ownerName" type="text" class="gridfield " id="ownerName" value="<?php if($OpsAssignTo!=''){ echo getUserName($OpsAssignTo); } ?>" readonly="true" displayname="Operation&nbsp;Person" autocomplete="off" onclick="function_assignTo();" /> 
...
<script>
function QuotaItinInfoSec() {
	// QuotaItinInfoUp,QuotaItinInfoDown,QuotaItinInfoSec,QuotaItinInfo_Hide
var x = document.getElementById("QuotaItinInfo_Hide");
var QuotaItinInfoUp = document.getElementsByClassName("QuotaItinInfoUp");
var QuotaItinInfoDown = document.getElementsByClassName("QuotaItinInfoDown");
if (x.style.display === "block") {
	x.style.display = "none";
	$('.QuotaItinInfoUp').hide();
	$('.QuotaItinInfoDown').show();
} else {
	x.style.display = "block";
	$('.QuotaItinInfoDown').show();
	// $('.QuerySecUp').hide();
...
showCNBCWBRoom();
function showCNBCWBRoom() {
var child = $('#child').val();

if(child>0){
	$('#shownotcnbcwb1').css("display","block");
	$('#shownotcnbcwb2').css("display","block");
}
...
<script>
function PriorityPreferenceSec() {
	
// PriorityPreferenceUp,PriorityPreferenceDown,PriorityPreferenceSec,PriorityPreference_Hide
var x = document.getElementById("PriorityPreference_Hide");
var PriorityPreferenceUp = document.getElementsByClassName("PriorityPreferenceUp");
var PriorityPreferenceDown = document.getElementsByClassName("PriorityPreferenceDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.PriorityPreferenceUp').show();
	$('.PriorityPreferenceDown').hide();
} else {
	x.style.display = "none";
	$('.PriorityPreferenceDown').hide();
	// $('.QuerySecUp').hide();
...
<div style="display: flex;width: 100%;">
	...
	<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Priority Preference</span>
	</div>
	<div style="text-align: right; width: 50%;">
	<span>
	<img class="PriorityPreferenceDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="PriorityPreferenceSec()">
		<img class="PriorityPreferenceUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="PriorityPreferenceSec()">
	</span>
	</div>
</div>
	...
	<div style=" border:1px #ccc solid; padding:10px;border-top:0px; display:block;display:none;" id="PriorityPreference_Hide" >
	<div id="show-info" class="" style="display:none1">
	
	<table width="100%" border="0" cellpadding="0" cellspacing="0">

		<!--Add Budget and language -->
			<tr>
				<td width="48%" align="left" valign="top"><div class="griddiv" style="width: 96%; margin-bottom:5px;">
				<label>
				<div class="gridlable">Budget  </div>
				<input style="border-radius: 5px; margin: 5px 0px;border: 1px solid #ccc;" name="budget" type="number" class="gridfield" id="budget"  displayname="Budget"   value="<?php echo $budgetCost; ?>" />
				</label>
				</div></td>
				...
				<select id="preferredLang" name="preferredLang" class="gridfield" displayname="Preferred Language">
		<?php
		$rs='';
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<!--Add More Emails -->
			<tr>
				...
				<option value="<?php if($leadId1!=''){echo $editleadsource;}else{ echo strip($resListingl['id']);} ?>" <?php 				
				if($leadId1!=''){
					if($resListingl['id']==$editleadsource){ ?>selected="selected"<?php }
				}else{
					if($resListingl['id']==$leadsource){ ?>selected="selected"<?php }
				}?>>
				
				<?php echo strip($resListingl['name']); ?>
				</option>
				<?php } ?>
				</select>
				</label>
...
<script>
function HotelPreferenceSec() {
	
// HotelPreferenceUp,HotelPreferenceDown,HotelPreferenceSec,HotelPreference_Hide
var x = document.getElementById("HotelPreference_Hide");
var HotelPreferenceUp = document.getElementsByClassName("HotelPreferenceUp");
var HotelPreferenceDown = document.getElementsByClassName("HotelPreferenceDown");
if (x.style.display === "none") {
	x.style.display = "block";
	$('.HotelPreferenceUp').show();
	$('.HotelPreferenceDown').hide();
} else {
	x.style.display = "none";
	$('.HotelPreferenceDown').hide();
	// $('.QuerySecUp').hide();
...
<!-- Started hotel prefrance sec -->
<div style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;    border-top-right-radius: 5px;border-top-left-radius: 5px;margin-top: 20px;
" onclick="$('#showmorefield1').toggle();">
	
<div style="display: flex;width: 100%;">
	<div style="text-align: left;width: 50%;">
	<img style="font-size: 14px;position: relative;top: 0px;height: 20px;" src="images/preferences-dom.png"> 
	<span style="font-size: 14px;font-weight: 500;position: relative;top: -3px;left: 10px;color: hsl(33 95% 68% / 1);">Hotel Preference</span>
	</div>
	<div style="text-align: right; width: 50%;">
	<span>
	<img class="HotelPreferenceDown errow-drop" style="" src="images/down-arrow-30.png"  onclick="HotelPreferenceSec()">
		<img class="HotelPreferenceUp errow-drop" style="display:none" src="images/collapse-arrow.png"  onclick="HotelPreferenceSec()">
	</span>
	</div>
</div>
...
<div style=" border:1px #ccc solid; padding:10px;border-top:0px; display:block;display:none;" id="HotelPreference_Hide" >
	<div id="show-info" class="" style="display:none1">
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
...
<tr id="hotel_type" style="display: flex;">
...
<td width="50%" align="left" valign="top"><div class="griddiv"><label><div class="gridlable">Meal Plan<span class=""></span></div>

<select id="mealPlanId" name="mealPlanId" class="gridfield " displayname="Meal Plan" autocomplete="off"   style=" border-radius: 5px; margin: 10px 0px;border: 1px solid #ccc;">
<option value="0" >All</option>
<?php
$rs=GetPageRecord('*',_MEAL_PLAN_MASTER_,' deletestatus=0 and status=1 and name!="" order by name asc');
while($resListing=mysqli_fetch_array($rs)){
...
<!-- Ended hotel prefrance sec -->
	...
	<tr id="service_selection_box" style="width: 68%;position: relative;bottom: 270px;display:none;">
		...
		<td colspan="2" >
				...
				<div id="toppppq" style="background-color: white; padding:5px 0px 0px 20px;border: 1px #ccc solid;cursor: pointer;   border-top-right-radius: 5px;border-top-left-radius: 5px" onclick="$('#showmorefield1').toggle();">
					<img style="font-size: 14px;position: relative;top: 0px;height: 20px;" src="images/information-dom.png"> 
  ...
  <tr>
	...
	<textarea name="description" rows="10" class="gridfield" id="description">
...
<script type="text/javascript">
			...
			function gitcodefun(queryType){ 
				...
				if(queryType==1){
					document.getElementById("TotalDayNight").style.display = "block";
					document.getElementById("TotalDayNightDays").style.display = "none";
					$('#gitgroupcode').hide();
					...
					// ServiceSelectionSec();
					document.getElementById("main-service-selection").style.marginBottom = "250px";
					...
					$('#duplicateQuery').hide();
						...
						document.getElementById("main-service-selection").style.marginBottom = "0px";
						document.getElementById("TotalDayNight").style.display = "none";
						document.getElementById("TotalDayNightDays").style.display = "block";
						$('#gitgroupcode').hide();
						$('#discountslabouter').hide();
						$('#gitgroupName').hide();
						$('#discountSlabs').val('');
