...
<td width="7%">
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Active/InActive" onclick="masters_alertspopupopen('action=mastersdelete&name=<?php echo urlencode($pageName); ?>','600px','auto');" />
	<?php } ?>
	</div>
...
.dropdown-content,.searchbtnmain,.bluembutton,.topsearchfiledmain { 
    font-size: 12px;
        ...
        <td><a class="bluembutton" style="background-color: #1fc277 !important; border: 1px solid #1fc277 !important;"  href="<?php echo $fullurl; ?>travrmimports/itinerary-description-import-format.xls" ><i class="fa fa-download" aria-hidden="true"></i>Download Format</a><td>
...
<input name="module" id="module" type="hidden" value="<?php echo clean($_GET['module']); ?>" />
    ...
    <th align="left" class="header" >From </th>
	<th align="left" class="header" >To</th>
...
$where='where 1 '.$wheresearch2.' order by add_date desc'; 
$page=$_GET['page'];
 //hotelcategorytype is new table
$targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 
$rs=GetRecordList($select,'itineraryDescriptionMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
while($resultlists=mysqli_fetch_array($rs[0])){ 
    
	$fromDest = $resultlists['fromDestinationId'];
	$toDest = $resultlists['toDestinationId'];
?>
  <tr>
  ...
  <td align="center" valign="middle"><?php if($editpermission==1){ ?><input type="checkbox" id="c<?php echo $no; ?>" name="check_list[]" class="chk"  value="<?php echo encode($resultlists['id']).','.$resultlists['status']; ?>"/>
    ...
    <td align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','1000px','auto');" ><?php if($resultlists['image']!=''){ ?><img src="packageimages/<?php echo $resultlists['image']; ?>" width="75" height="58" /><?php } ?></div></td>
    <td width="10%" align="left"><?php echo getDestination($fromDest); ?></td>
	<td width="10%" align="left"><?php echo getDestination($toDest); ?></td>
	<td width="60%" align="left" title="<?php echo strip_tags($resultlists['description']); ?>">
	    <?php echo stripslashes($resultlists['description']); ?>
	</td>
	<td width="5%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','1000px','auto');" >Edit</div></td>
