...
if($_REQUEST['visaId']!=''){  
	$aaaaaa=GetPageRecord('*','visaCostMaster',' id="'.decode($_REQUEST['visaId']).'"'); 
	$visaCostData=mysqli_fetch_array($aaaaaa);
}
?>
...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="93%" align="left"><?php echo 'Visa Cost' ?>:&nbsp;<?php echo $visaCostData['name']; ?></td>
  </tr>
...
<div id="addTriffRoom" style="display:nxone;">
	<div class="addGreenHeader">Add VISA Cost</div>
	...
	<div class="gridlable">Visa&nbsp;Name<span class=""></span></div> 
		<input type="text" name="visaName" id="visaName" class=" gridfield" value="<?php echo $visaCostData['name']; ?>">
	</label>  
	</div></td>
	...
	<div class="gridlable">Country<span class="redmind"></span></div> 
		<select id="countryId" name="countryId"  displayname="country Id" class="validate gridfield" autocomplete="off" > 
	...
	<div class="gridlable">Visa&nbsp;Type<span class="redmind"></span></div> 
		<select id="visaTypeId" name="visaTypeId"  displayname="Visa Type" class="validate gridfield" autocomplete="off" > 
				<?php   
				$rs='';   
	...
	<td width="100" align="left"><div class="griddiv" >
				...
				<select id="entryType" name="entryType"  displayname=" EntryType " class=" gridfield" autocomplete="off" > 
				
				<option value="1" <?php if($visaCostData['entryType']==1){ ?> selected="selected" <?php } ?> >Single Entry</option>
				<option value="2" <?php if($visaCostData['entryType']!=1){ ?> selected="selected" <?php } ?>>Multiple Entry</option>
				
				</select></label>
				</div>			
			
		</td> 
		<td width="100" align="left" style="display:none2;"><div class="griddiv" > 
	<label> 
	<div class="gridlable">Validity<span class=""></span></div> 
		<input type="text" name="validity" id="validity" class=" gridfield" value="<?php echo $visaCostData['validity']; ?>">
	</label>  
	</div></td>
		  	...
		  	<td width="100" align="left"><div class="griddiv" >
				...
				<?php   
				$rs='';   
				$rs=GetPageRecord('*',_SUPPLIERS_MASTER_,' deletestatus=0 and name!="" and status=1 and visaType=11 order by name asc'); 
	...
	<input name="toDate" type="text" id="toDate" class="gridfield calfieldicon validate" displayname="Rate Valid To" autocomplete="off" style="width: 100%;"/>
		  ...
		  <td width="100" align="left"  >
			...
			<td width="100" align="left"  >
				<div class="griddiv">
					<label>
						...
						<input type="text" name="embassyFee" id="embassyFee" class="gridfield" displayname="Embassy Fee">
					</label>
				</div>
			</td> 
			<td width="100" align="left"  >
				<div class="griddiv">
					<label>
						...
						<input type="text" name="vfsCharges" id="vfsCharges" class="gridfield" displayname="VFS Charges">
					</label>
				</div>
			</td> 
			...
			<td width="100" align="left" valign="middle"  ><input type="button" name="Submit" value="   Save   " class="bluembutton"  onclick="formValidation('addVisaPrice','saveflight','0');"> 
			  <input name="action" type="hidden" id="action" value="addVisaRate">
			  <input name="serviceid" type="hidden" id="serviceid" value="<?php echo $visaCostData['id']; ?>"></td>
			</tr> 
		</tbody></table>
		</form>
	</div>
...
<div id="loadVISArate"></div> 

<script>   
function funloadVisaRate(){ 
$('#loadVISArate').load('loadvisaratemaster.php?serviceid=<?php echo decode($_REQUEST['visaId']); ?>'); 
} 
