...
// }
if($roleId==1){
$DataQuery="SELECT id,guideQuoteId,quotationId,queryId,name,mobileNo,finalActive,appStatus,allocationStatus,TourManagerId FROM tourManagerAllocation WHERE finalActive=1 and appStatus=2 and allocationStatus=2 and TourManagerId='".$driverId."' '".$dateQ."' order by id desc";
...
// $DataQuery="SELECT id,transferQuotId,quotationId,queryId,name,mobileNo,finalActive,appStatus,allocationStatus FROM driverAllocationDetails WHERE finalActive=1 and appStatus=2 and allocationStatus=2 and driverId='".$driverId."' '".$dateQ."' order by id desc";

$DataQuery="SELECT * FROM driverAllocationDetails WHERE driverId='".$driverId."' and fromDate ='".$dateQ."' and finalActive=1 and appStatus=2 and allocationStatus=2";
$pendingStatusDataaaq=mysqli_query(db(),$DataQuery) or die(mysqli_error(db()));
while($pendingStatusDataaa=mysqli_fetch_array($pendingStatusDataaaq)){
...
$DataQuery="SELECT * FROM guideAllocation WHERE finalActive=1 and appStatus=2 and allocationStatus=2 and GuideId='".$driverId."' and fromDate ='".$dateQ."' order by id desc";
$pendingStatusDataaaq=mysqli_query(db(),$DataQuery) or die(mysqli_error(db()));
while($pendingStatusDataaa=mysqli_fetch_array($pendingStatusDataaaq)){
$guideQuoteId=$pendingStatusDataaa['guideQuoteId'];
//queryData
