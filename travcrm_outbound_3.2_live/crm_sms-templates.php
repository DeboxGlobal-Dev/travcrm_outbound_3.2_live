...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable">
   ...
   <tr>
     ...
     <th width="724" align="left" class="header" >SMS </th>

     <th width="184" align="center" class="header">Status</th>
...
$where='where deletestatus=0 and  userId='.$loginusersuperParentId.' order by id desc';   
 $page=$_GET['page'];
$targetpage=$fullurl.'setupsetting.crm?module=smstemplates&records='.$limit.'&'; 
$rs=GetRecordList($select,_SMS_TEMPLATE_MASTER_,$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
  <tr>
    ...
    <td align="left" valign="top"><div class="bluelink"<?php if($editpermission==1){ ?>onclick="edit('<?php echo encode($resultlists['id']); ?>');"<?php }  ?> ><?php echo strip($resultlists['smsbody']); ?></div></td>

    <td align="center" valign="top"><?php if($resultlists['status']==1){ ?><div class="statusactive">Active</div><?php } else { ?>
...
?>
    ...
    <?php if($deletepermission==1){ ?>  <a class="redtextc" onclick="$('#dids').val('<?php echo encode($resultlists['id']); ?>');alertspopupopen('action=deletesmstemplate','600px','auto');" >Delete</a><?php } ?>      <?php } ?></td>
    </tr> 
