<?php
  ...
  if($balitourid==''){
    ...
    if($lastBaliId > 0){
      ...
      header('location: '.$fullurl.'webquery.php?fromDate='.$fromDate.'&toDate='.$toDate.'&email='.$email.'&fname='.$fname.'&city='.$city.'&toDest='.$toDest.'&phone='.$mobileNumber.'&subject='.$subject.'&ip='.$ip.'&Website='.$Website.'&LeadTo='.$LeadTo.'&url='.$url.'&date_change='.$date_change.'&query_date='.$query_date.'&exploring='.$exploredesti.'&queryPriority=2&websitelead='.$Websitelead.'&baliId='.$baliId.'&fromDest='.$fromDest.'&leavingForm='.$leavingForm.'&adult='.$adults.'&child='.$children.'&infant='.$infant.'&tourtype='.$tot.'&hotelCategory='.$hotelstars.'&flight_requirement_97='.$flight_requirement_97.'&flight_requirement='.$flight_requirement.'&budget='.$budget.'&specific_requirements='.$specific_requirements.'');
