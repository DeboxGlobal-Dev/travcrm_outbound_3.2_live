...
<table width="100%" border="1" bordercolor="#ccc" id='dataTableDiv' cellpadding="0" cellspacing="0" class="tablesorter gridtable">
   ...
   <tr>
	 ...
	 <th align="left" class="header" style="min-width: 65px !important;">Due&nbsp;Date</th>
...
if($_REQUEST['searchBy']=='1'){ 
  ...
  $where=' status=1 and scheduleStatus=1 and dueDate < "'.date('Y-m-d').'"  and quotationId in ( select id from '._QUOTATION_MASTER_.' where 1 and status=1 '.$travelDateQuery.' '.$agentQuery.' and queryId in ( select id from queryMaster where queryStatus=3 )) '.$datefilterQuery.' group by paymentId order by dueDate desc'; 
			...
			<td align="right"> <strong  style="color:#6ebac7">
			  <?php   
				...
				if(mysqli_num_rows($r2) > 0){ 
					...
					$r222=GetPageRecord('SUM(amount) as receivedAMT','agentPaymentMaster','paymentId="'.$agentPaymentData['id'].'" ');
					$rsPaymentData = mysqli_fetch_array($r222); 
					$pendingCost = $finalCost-$rsPaymentData['receivedAMT'];
