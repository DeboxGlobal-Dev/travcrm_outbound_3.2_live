...
deleteRecord(_MEETINGS_MASTER_,$wheredel);
...
$lastqueryidmain= addlistinggetlastid(_MEETINGS_MASTER_,$namevalue);
}
...
<option value="2" <?php if(2==$clientType){ ?>selected="selected"<?php } ?>>B2C</option> 
	...
	<div class="griddiv" id="selectclientbox" style="overflow: unset;">
		<div onclick="openselectCompanypop('action=addAgentClienttoQuery&actionType=addserviceagentclient&addTo=meetings','700px');" style="position: absolute;cursor: pointer;top: 22px;right: 3px;background: #233a49;color: white;padding: 8px 5px;">Add New</div>
...
$start=strtotime('00:00');
   ...
   <option value="<?php echo date('g:i A',$i); ?>" <?php if('9:00 AM'==date('g:i A',$i) && $_REQUEST['id']==''){ ?> selected="selected"<?php } ?> <?php if($editstarttime==date('g:i A',$i)){ ?> selected="selected"<?php } ?>><?php echo date('g:i A',$i); ?></option>;
	...
	<div class="griddiv">
	<label>
	<div class="gridlable">Meeting&nbsp;Duration</div>
	<select id="timeDuration" name="duration" class="gridfield validate" displayname="Meeting Duration" autocomplete="off" onchange="setendtime(this.value);" > 
<option value="">Select</option>
<?php
    for ($i=5;$i<=300;$i = $i + 5) { ?>
...
<?php } ?>
	...
	<div class="griddiv"><label>
	<div class="gridlable"> Meeting Agenda <span class="redmind"></span></div>
