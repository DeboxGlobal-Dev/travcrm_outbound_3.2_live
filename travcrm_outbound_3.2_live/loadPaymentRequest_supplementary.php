...
deleteRecord(_SUP_PAYMENT_REQUEST_,' id = '.$_REQUEST['deleteId'].' and queryId='.$_REQUEST['id'].''); 
}

if($_REQUEST['savereqeust']=='1'){
$namevalue = 'status=1,invoiceGenerateDate="'.date('Y-m-d').'"';  
$where='queryId='.$_REQUEST['id'].'';  
$update = updatelisting(_SUP_PAYMENT_REQUEST_,$namevalue,$where); 
}
...
$rs2=GetPageRecord($select2,_SUP_PAYMENT_LIST_MASTER_,$where2); 
...
$rs4=GetPageRecord($select4,_SUP_PAYMENT_LIST_MASTER_,$where4); 
...
?>
    ...
    <td width="9%" align="right" valign="top"><input name="addnewuserbtn" type="button" class="greenmbutton3 submitbtn" id="addnewuserbtn" value="Update Payment"  onclick="alertspopupopen('action=supplementarymakepaymentrequestpayment&queryId=<?php echo ($_REQUEST['id']); ?>','500px','auto');" /></td>
  </tr>
</table>
...
?>
  <tr>
    ...
    <td align="center"><?php  echo  ($dmcroommastermain['gst']); ?>%</td>
<?php 
$select=''; 
$where=''; 
$rs='';  
...
?>
    ...
    <td align="center"><a onclick="alertspopupopen('action=adddmcpaymentrequest&queryId=<?php echo clean($_GET['id']); ?>&id=<?php  echo ($dmcroommastermain['id']); ?>','600px','auto');">Edit</a></td>
...
<div style="padding:20px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable">
   ...
   <tr>
      ...
      <th align="left" class="header">GST Type </th>
      ...
      <th align="right" class="header">GST</th>
      <?php 
$select=''; 
$where=''; 
$rs='';  
...
$select='*';    
$where=' gstValue='.$dmcroommastermain['gstTypeId'].'';  
...
echo  $resListing['name'];
    ...
    <td align="left"><?php  echo  ($dmcroommastermain['gst']); ?>%</td>
...
echo  $resListing['name'];
    ...
    <td align="right"><?php $totalsub=$dmcroommastermain['rate']*$dmcroommastermain['pax']; echo $totalsubgst=$totalsub*$dmcroommastermain['gst']/100; ?></td>
    <?php 
$select=''; 
$where=''; 
$rs='';  
...
?>
    <?php  } ?>
    <td align="right"><?php $totalsub=$dmcroommastermain['rate']*$dmcroommastermain['pax']; $totalsubgst=$totalsub*$dmcroommastermain['gst']/100; echo  $totalsub+$totalsubgst; ?></td>
...
<?php } ?>
 ...
 dmcpaymentrequestrate();
	...
	<select id="gstTypeId2" name="gstTypeId2" class="gridfield validate" displayname="GST Type" autocomplete="off" onchange="setgst();">
	 <option value="">Select</option>
 <?php 
$select=''; 
$where=''; 
...
<?php } ?>
    ...
    <td valign="top"><div class="griddiv" style="width: 50px;"><label>
  
	<div class="gridlable">GST %</div>
	<input name="gst" type="text" class="gridfield" id="gst"   maxlength="3" displayname="GST" style="width:50px;"  autocomplete="off" onkeyup="numericFilter(this);" value="<?php echo $gst; ?>"  />
	</label>
...
<?php } ?>
	...
	<input name="rate2"   type="text" class="gridfield" id="rate2"  style="width: 75px;"  maxlength="12" displayname="Rate"  autocomplete="off" onkeyup="numericFilter(this);dmcpaymentrequestrate();" value="<?php echo $rate; ?>"  />
	</label>
	<script>
	...
	<input name="totalRate"   type="text" class="gridfield" id="totalRate"   displayname="Total" style="width:60px;"  autocomplete="off" value=""  readonly="readonly"   />
	</label>
	 
 </div></td>
    <td valign="top"><div class="griddiv"><label>
  
	<div class="gridlable">GST <span class="redmind"></span></div>
	<input name="gsttotal"   type="text" class="gridfield" id="gsttotal"   maxlength="3" displayname="GST" style="width:60px;" readonly="readonly"  autocomplete="off" onkeyup="numericFilter(this);dmcpaymentrequestrate();" value="<?php echo $pax; ?>"  />
	</label>
    ...
    <td valign="top">  <input name="action" id="action" type="hidden" value="addsupplementarypaymentrequest" />
