					...
					<tr>
								...
								<tr>
									...
									<td >
										<table width="100%" border="0" cellpadding="0" cellspacing="0">
											<tr>
												<td><input name="searchField" type="text"  class="topsearchfiledmain" id="searchField" style="width:100px;" value="<?php echo $searchField; ?>" placeholder="Docket Id" onkeyup="numericFilter(this);"/></td>
												<td style="padding:0px 0px 0px 5px;" ><input name="searchFieldcommon" type="text"  class="topsearchfiledmain" id="searchFieldcommon" style="width:110px;" value="<?php echo $searchFieldcommon; ?>" size="100" maxlength="100" placeholder="Docket Name"  /></td>
						...
						<form id="listform" name="listform" method="get">
									...
									<table width="100%" border="0" cellpadding="5" cellspacing="0">
										<tr>
											...
											<td width="10%" align="left" valign="top"><div style="padding:10px; color:#fff; font-size:12px; background-color:#2ca1cc;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=10 and moduleType=5  and deletestatus=0  '); ?></div>
											...
											<td width="10%" align="left" valign="top"><div style="padding:10px; color:#fff; font-size:12px; background-color:#FF6600;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=2 and moduleType=5  and deletestatus=0 '); ?></div>
											...
											<td width="10%" align="left" valign="top"><div style="padding:10px; color:#fff; font-size:12px; background-color:#82b767;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=3 and moduleType=5  and deletestatus=0 '); ?></div>
											...
											<td width="10%" align="left" valign="top"><div style="padding:10px; color:#fff; font-size:12px; background-color:#2ca1cc;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=6  and moduleType=5 and deletestatus=0 '); ?></div>
											...
											<td width="10%" align="left" valign="top" ><div style="padding:10px; color:#fff; font-size:12px; background-color:#9466be;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=7  and moduleType=5 and deletestatus=0 '); ?></div>
											...
											<td width="10%" align="left" valign="top"><div style="padding:10px; color:#fff; font-size:12px; background-color:#c75858;border-left: 4px solid #000;border-radius: 4px; overflow: hidden;">
												<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=20  and moduleType=5 and deletestatus=0 '); ?></div>
											...
											<div style="font-size:20px; font-weight:600;"><?php echo countlisting('id','queryMaster','where queryStatus=4  and moduleType=5 and deletestatus=0 '); ?></div>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0"  id="mainsectiontable" class="table table-striped table-bordered">
								<thead>
									<tr>
										<th align="center" valign="middle" class="header" width="5%">&nbsp;</th>
										<th align="left" class="header" width="10%">Docket&nbsp;ID</th>
										<th align="left" class="header" >Lead Pax Name</th>
										...
										<th align="left" class="header" width="10%">Ops.Person</th>
										<th align="center" class="header" width="10%">Status</th>
									</tr>
								</thead>
								<tbody>
									<?php
									$moduleTypes=' and	moduleType=5';
									$no=1;
									$select='*';
									$where='';
									$rs='';
									...
									$where=' where '.$wheresearch.' '.$whereFromDate.' '.$daterangeQuery.' '.$mainwhere.' '.$destination.'  '.$querystatus.' '.$queryDateDash.' '.$moduleTypes.' and deletestatus=0  ORDER BY queryOrder DESC, dateAdded DESC ';
									//and toDate>="'.date('Y-m-d').'" 
									}
									if($searchField!=''){
										$where = '';
										$where=' where 1 '.$mainwhere.' and deletestatus=0 and moduleType=5 ORDER BY queryOrder DESC, dateAdded DESC ';
									}
									$page=$_GET['page'];
									// echo $where;
									$targetpage=$fullurl.'showpage.crm?module=docket&records='.$limit.'&searchField='.$searchField.'&assignto='.$_GET['assignto'].'&queryshow='.$_REQUEST['queryshow'].'&searchdate='.$_REQUEST['searchdate'].'&thismonth='.$_REQUEST['thismonth'].'&';
									...
									<tr <?php if($resultlists['queryStatus']==20){ ?>style="background-color: #fff2f2;"<?php } ?>>
										<td align="center" valign="middle" style=" width:30px;">
											...
											<i class="fa fa-pencil-square" aria-hidden="true" onclick="editDocket('<?php echo encode($resultlists['id']); ?>');"  style="cursor:pointer;"></i>
											<?php } ?>
										</td>
										<td align="left">
											<div class="bluelink" style="position:relative; padding-right:10px; font-weight:500; color:#45b558 !important; " onclick="viewDocket('<?php echo encode($resultlists['id']); ?>');">
												<?php echo makeDocketId($resultlists['displayId']); ?>
											...
											<?php if(countSupplierunreadMails($resultlists['id'])!=0){ ?><div class="numberbubbol" style="background-color:#359ec5;left: 58px; top: 22px; margin-bottom: 3px;"><?php echo countSupplierunreadMails($resultlists['id']); ?></div>
											<?php } ?>
											</div>   
										</td>
										<td align="left" ><?php echo $resultlists['leadPaxName'];  ?></td>
										...
										<td align="center"style="width:50px;">
											<select id="changeStatus<?php echo $resultlists['id']; ?>" class="assignquery" style="width:105px;background-color:<?php if($resultlists['queryStatus'] == 4){ echo '#ff0000	'; }elseif($resultlists['queryStatus'] == 3){ echo '#45b558'; }elseif($resultlists['queryStatus'] == 7){ echo '#233a49'; }else{ echo '#7a96ff'; } ?>; " onchange="changeStatus(this.value,<?php echo $resultlists['id']; ?>);"  >
											<option value="1" <?php if($resultlists['queryStatus'] =='1'){ ?>selected<?php } ?>>Assigned</option>
											<option value="3" <?php if($resultlists['queryStatus'] =='3'){ ?>selected<?php } ?>>Confirmed</option>
											<option value="7" <?php if($resultlists['queryStatus'] =='7'){ ?>selected<?php } ?>>Follow-up</option>
											<option value="20" <?php if($resultlists['queryStatus'] =='4'){ ?>selected<?php } ?>>Lost</option>
											<!-- <option value="">All Status</option> -->
											<!-- <option value="2">Reverted</option> -->
											<!-- <option value="20">Cancelled</option> -->
											<!-- <option value="5">Time Limit Booking</option> -->
											<!-- <option value="6">Options Sent</option> -->
											<!-- <option value="10">Created</option> -->
											</select>
									...
									<script>
										function editDocket(id){
											setupbox('showpage.crm?module=docket&edit=yes&id='+id+'');
										}
								...
								<div style="display:none" id="changeStatusDiv"></div>
								<script type="text/javascript">
									function changeStatus(queryStatus,queryId) {
										if(queryStatus == 1){
											$('#changeStatus'+queryId).css('background-color','#7a96ff');
										}if(queryStatus == 3){
											$('#changeStatus'+queryId).css('background-color','#45b558');
										}if(queryStatus == 7){
											$('#changeStatus'+queryId).css('background-color','#233a49');
										}if(queryStatus == 4){
											$('#changeStatus'+queryId).css('background-color','#ff0000');
										}
										$('#changeStatusDiv').load('docket_frmaction.php?action=changeDocketStatus&queryId='+queryId+'&queryStatus='+queryStatus);
									}
								</script>
								<?php if($no==1){ ?>
