...
<input name="table" id="table" type="hidden" value="<?php echo _GUIDE_CAT_MASTER_; ?>" />
<table border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable">
    ...
    <label for="checkAll"><span></span>&nbsp;</label></th> 
			<th align="left" width="5%" class="header" >#&nbsp;</th>
			<th align="left" width="20%"  class="header" >Name </th>   
			<th align="left" width="50%" class="header" >Price</th> 
		</tr>
	</thead>
  	<tbody>
		<?php 
		...
		$where='where 1 '.$wheresearch.' order by id desc, status desc'; 
		$page=$_GET['page']; 
		$targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 
		$rs=GetRecordList($select,_GUIDE_CAT_MASTER_,$where,$limit,$page,$targetpage); 
		$totalentry=$rs[1]; 
		$paging=$rs[2]; 
