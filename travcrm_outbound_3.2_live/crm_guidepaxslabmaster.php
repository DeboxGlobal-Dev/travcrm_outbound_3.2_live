  ...
  <tr>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add <?php echo $pageName; ?>" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>','600px','auto');" /></td> <?php } ?>
      </tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
     ...
     <th width="4%" align="center" valign="middle" class="header" >
	  ...
	  <th align="left" class="header" >Min&nbsp;Pax</th>
    <th align="left" class="header" >Max&nbsp;Pax</th>
...
if($_GET['keyword']!=''){
	$wheresearch=" and ( guideSlabName like '%".$_GET['keyword']."%' ) ";
}
if($_REQUEST['status']!=''){
	$wheresearch2 = " and status ='".clean($_REQUEST['status'])."' ";
}
//$wheresearch3=' ( addedBy = '.$_SESSION['userid'].'  or assignTo = '.$_SESSION['userid'].')';  
//$where='where  guideSlabName!=""  '.$wheresearch.' '.$wheresearch2.' '.$wheresearch3.' and deleteStatus=0 order by guideSlabName asc'; 
...
//$targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 
 ...
 $where='where  guideSlabName!=""  '.$wheresearch.''.$wheresearch2.' and deleteStatus=0 order by guideSlabName asc'; 
 
 $targetpage=$fullurl.'showpage.crm?module='.$_GET['module'].'&records='.$limit.'&'; 

$rs=GetRecordList($select,'guidePaxSlabMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
  <tr>
	...
	<td width="5%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&editId=<?php echo $resultlists['id'];?>','600px','auto');" ><?php echo $resultlists['guideSlabName']; ?></div></td>
	<td width="5%" align="left"><?php echo $resultlists['minPax']; ?></td>
	<td width="5%" align="left"><?php echo $resultlists['maxPax']; ?></td>	
