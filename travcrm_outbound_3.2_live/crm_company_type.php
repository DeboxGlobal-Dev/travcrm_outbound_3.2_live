...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Business&nbsp;Type','600px','auto');" />
	<?php } ?>
	</div>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add <?php echo "Business" ;////$pageName; ?>" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> <?php } ?>
      </tr>
		...
		<input name="action" id="action" type="hidden" value="deletecompany_type" />
		<input name="table" id="table" type="hidden" value="businessTypeMaster" />
   ...
   <tr>
     ...
     <th align="left" class="header" ><?php echo $pageName; ?> </th>
...
$rs=GetRecordList($select,'businessTypeMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1];
$paging=$rs[2]; 
while($resultlists=mysqli_fetch_array($rs[0])){ 
	...
	$modifyDate=clean($resultlists['modifyDate']); ?>
  	<tr>
    <td width="2%"><?= $no ?></td>  
    <td align="center" width="2%" valign="middle"><input type="checkbox" id="c<?php echo $no; ?>" name="check_list[]" class="chk"  value="<?php echo encode($resultlists['id']); ?>"/><label for="c<?php echo $no; ?>"><span></span>&nbsp;</label><?php ?></td>
    <td width="15%" align="left">
	<div class="bluelink"   onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" >
	<?php echo $resultlists['name']; ?>
	</div>  
	...
	<td width="27%" align="left"><?php $select=''; 
$where2=''; 
$rs2='';  
