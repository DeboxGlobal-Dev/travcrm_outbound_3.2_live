			...
			<div class="headingm" style="margin-left:30px;">
				...
				<div id="deactivatebtn"style="display:none">
					<?php if($deletepermission==1){ ?>  
						<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Delete" onclick="masters_alertspopupopen('action=mastersdelete&name=<?php echo str_replace(" ","%20",$pageName); ?>','600px','auto');" /> 
					<?php } ?>
				</div> 
			</div>
		</td>
...
<input name="table" id="table" type="hidden" value="<?php echo _GUIDE_SER_PRICE_MASTER_; ?>" />
<table border="0" cellpadding="0" cellspacing="0" class="tablesorter gridtable">
    ...
    <label for="checkAll"><span></span>&nbsp;</label></th> 
			...
			<th align="left" class="header" >Rules</th>
			<th align="left" class="header" >Category </th>
			<th align="left" class="header" >Sub Category</th> 
			<th align="left" class="header" >Price</th> 
  	...
  	<tbody>
		...
		$rs=GetRecordList($select,_GUIDE_SER_PRICE_MASTER_,$where,$limit,$page,$targetpage); 
		$totalentry=$rs[1]; 
		$paging=$rs[2]; 
    ...
    <label for="c<?php echo $no; ?>"><span></span>&nbsp;</label><?php } ?></td>
			...
			<td align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&editId=<?php echo $resultlists['id']; ?>','400px','auto');" >
				<?php  
				echo $resultlists['name'];
				?></div>   
			</td> 
			<td align="left">
				<?php 
				$select2='*';  
				$where2=' id ="'.$resultlists['category'].'"'; 
				$rs2=GetPageRecord($select2,_GUIDE_CAT_MASTER_,$where2); 
			...
			<td align="left">
				<?php 
				$select21='*';  
				$where21=' id ="'.$resultlists['subcategory'].'"'; 
				$rs21=GetPageRecord($select21,_GUIDE_SUB_CAT_MASTER_,$where21); 
				$editresult21=mysqli_fetch_array($rs21); 
				echo $editresult21['name'];
				?>
			</td>
			<td align="left"><?php echo $resultlists['price']; ?></td> 
