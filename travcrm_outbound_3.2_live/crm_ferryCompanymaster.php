...
<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Ferry&nbsp;Company&nbsp;Master','600px','auto');" />
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add Ferry Company" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','700px','auto');" /></td>  <?php } ?>
      </tr>
...
<input name="action" id="action" type="hidden" value="deleteFerryCompany" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
	 ...
	 <th align="left" class="header" >Ferry Website</th>
...
$rs=GetRecordList($select,'ferryCompanyMaster',$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
  <tr>
    ...
    <?php 
      $alldest = explode(',',$resultlists['location']);
	...
	<strong>Cont.&nbsp;No:&nbsp;</strong><?php echo decode($resultlists['phone']); ?><br>
	<strong>Email&nbsp;Id:&nbsp;</strong><?php echo decode($resultlists['email']); ?>
