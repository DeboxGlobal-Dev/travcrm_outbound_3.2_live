...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td><div class="headingm" style="margin-left:10px;"><span id="topheadingmain"><?php echo Airline;//$pageName; ?></span>
    <div id="deactivatebtn" style="display:none;">
     <?php if($deletepermission==1){ ?> 
    
    <input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Airline','600px','auto');" />
    <?php } ?>
    </div>
...
.dropdown-content,.searchbtnmain,.bluembutton,.topsearchfiledmain { 
    font-size: 12px;
        ...
        <?php   $dirname =  'log_airline/'; 
      $images = scandir($dirname);
      krsort($images);
      foreach (array_slice($images, 0, 5) as $file) {
          if (substr($file, -4) == ".log" ) {
              ?>
              <a href="<?php echo $fullurl; ?>log_airline/<?php echo $file; ?>" target="_blank"><?php echo $file; ?></a>
              <?php  
          }
      }
      ?>
       ...
       <td style="boarder-radius:10%">
        ...
        <td><input type="submit" name="Submit" value="Search" class="searchbtnmain"></td>
         ...
         <td  ><a href="<?php echo $fullurl; ?>travrmimports/airline-import-format.xls" class="bluembutton"  style="background-color: #1fc277 !important; border: 1px solid #1fc277 !important;"><i class="fa fa-download" aria-hidden="true"></i> Download Format</a>
         </td>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn"  type="button" class="bluembutton" id="addnewuserbtn" value="+ Add Airline" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> <?php } ?>
      </tr>
...
<input name="action" id="action" type="hidden" value="delete_packageairlinemaster" />
...
<table border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <thead>
      ...
      <tr>
            ...
            <th width="15%" align="center" class="header" >Airline Code</th>
            <th width="10%"  class="header" >Image</th>
            <th align="left" class="header" >Airline&nbsp;Name </th>
            <th width="10%"  align="left" class="header" >Status</th>
            <th width="5%" align="left" width="10%" class="header" >Rate&nbsp;Sheet</th>
      </tr>
   </thead>
  <tbody>
  <?php
    ...
    if($_GET['keyword']!=''){
        $wheresearch=" and flightName like '%".$_GET['keyword']."%'";
    }
    if($_REQUEST['status']!=''){
     ...
     $where='where flightName!=""  '.$wheresearch.''.$wheresearch2.' and deletestatus=0 order by flightName asc'; 
    ...
    $rs=GetRecordList('*',_PACKAGE_BUILDER_AIRLINES_MASTER_,$where,$limit,$page,$targetpage); 
    $totalentry=$rs[1]; 
    //print_r($resultlists);
    $paging=$rs[2]; 
    while($resultlists=mysqli_fetch_array($rs[0])){ 
        ...
        ?>
            <tr>
                ...
                <td align="center"><?php  echo $serviceCode = makeServiceCode('AL',$resultlists['displayId']);  ?></td>


                <td align="left"><?php if($resultlists['flightImage']!=''){ ?><img src="packageimages/<?php echo $resultlists['flightImage']; ?>" width="75" height="58" /><?php } ?></td>
                <td align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo ucfirst($resultlists['flightName']); ?></div>   </td>
                <td width="10%" align="center"  valign="middle" ><a href="showpage.crm?module=<?php echo $_REQUEST['module'];?>&amp;view=yes&amp;id=<?php echo encode($resultlists['id']);?>"><input name="addnewuserbtn" type="button" value="+ Add/View" class="bluembutton"></a></td>
...
<form action="frm_action.crm" method="post" enctype="multipart/form-data" name="importfrm" id="importfrm"  target="actoinfrm" style="display:none;">
 <input name="importAirlineMaster" id="importAirlineMaster" type="hidden" value="Y" /> 
 <input name="importAirlineMasterModule" id="importAirlineMasterModule" type="hidden" value="<?php echo clean($_GET['module']); ?>" />
