...
if ($_GET['id'] != '') {
	...
	$rsp = GetPageRecord('*',_PAYMENT_REQUEST_SECTION_MASTER_,'queryid="'.$resultpage['id'].'" and quotationId="'.$resultpageQuotation['id'].'"');
	$resultpaymentpage = mysqli_fetch_array($rsp);
	$selecta = '*';
	$wherea = 'paymentId="'.$resultpaymentpage['id'].'"';
	$rsa = GetPageRecord($selecta,_AGENT_PAYMENT_REQUEST_,$wherea);
	$requesetdata = mysqli_fetch_array($rsa);
	$TotalrecievedAmount = $requesetdata['finalCost']-$requesetdata['pendingCost'];

	$totalpendingamount = 0;
	$s = 1;
	$select2 = '*';
	$where2 = 'queryId="'.clean(decode($_GET['id'])).'"';
	...
	$s = 1;
	$select21 = '*';
	$where21 = 'paymentRequestId="' . $resultpaymentpage['id'] . '"  order by id ASC';
	$rs21 = GetPageRecord($select21, _PAYMENT_LIST_MASTER_, $where21);
	while ($listofpayment = mysqli_fetch_array($rs21)) {
	$totalpaid = $totalpaid + $listofpayment['amount'];
	}
	$select = '';
	$where = '';
	$rs = '';
		...
		<div class="innerdiv">
			<div class="contentbox" style="background-color: rgba(0,0,0,0.2);">
				<div class="lables">
					...
					<?php if ($resultpage['queryStatus'] == 3 && $resultpage['queryConfirmingDate'] || $resultpage['queryStatus'] == 20 && $resultpage['queryConfirmingDate']) { ?><div style="font-size:16px;" class="statustbs">Tour Id - <?php echo makeQueryTourId($resultpage['id']); ?></div>
			...
			<div class="heading" style="position:relative;">
				<table border="0" cellpadding="0" cellspacing="0" style="position:absolute; right:10px; top:67px;">
					...
					<td>
						...
						<?php }
						if ($resultpage['moduleType'] == '4') { ?>Add&nbsp;Package
						<?php } ?>" style="max-width: 119px; height:31px;" onClick="alertspopupopen('action=selectItineraryType&queryId=<?php echo $resultpage['id']; ?>&qtype=<?php echo $queryTypeId; ?>&moduleType=<?php echo $resultpage['moduleType']; ?>','450px','auto');" />
					</td>
					<?php } ?>
							...
							<div>
								<div style="font-size:12px; color:#CCC; position:relative;" class="statustbs2">
									...
									if ($resultpage['queryStatus'] == 6) {
										echo '<div class="assignquery">Options&nbsp;Sent</div>';
										$list_id = 'b09914105b';
									}
	...
	<div class="querywonlostclose" <?php if ($resultpage['queryStatus'] == 3) { ?>style="background-color:#82b767;" <?php } ?><?php if ($resultpage['queryStatus'] == 4 || $resultpage['queryStatus'] == 20) { ?>style="background-color:#c75858;" <?php } ?><?php if ($resultpage['queryStatus'] == 5) { ?>style="background-color:#4CAF50 ;" <?php } ?>>
		...
		<div style="font-size:12px; margin-bottom:5px;"><strong>End Date:</strong> <?php echo showdate($resultpage['followupdate']); ?></div>
		<?php } ?>
	...
	<div class="replyboxmain" id="replymainbox">
		...
		}elseif($_REQUEST['sendfeedbackform']!=''){
			$replymainbox = 1;
			$preview = url_get_contents($fullurl.'sendfeedbackmail.php?queryId='.decode($_REQUEST['id']).'&quotationId='.trim($_REQUEST['quotationId']));
		...
		<?php if($_REQUEST['voucherString']!= '') {  
			...
			<div style="font-size: 15px;padding: 10px 0;border: 1px solid #dadadabd;box-shadow: 1px 1px 10px #cccccc87; background-color:#fff;">
				<label for="isAttachement">
					<input type="checkbox" id="isAttachement"  name="isAttachement" value="1" style="display: inline-block;width: 15px;height: 15px;">&nbsp;&nbsp;&nbsp;Attache as PDF.</label>  
	...
	while ($querylisting = mysqli_fetch_array($rs)) {
		...
		if ($mailbodydetails != '') {
		?>
		<div class="querymaillisting" style="border-left:4px solid #<?php if ($querylisting['queryStatus'] == 1) {
		echo '2ca1cc';
		}
		...
		<div class="mailusersbox"><strong>Group: </strong><?php echo $resultpageemail['queryemail']; ?>
		<?php
		if ($querylisting['multiemails'] != '') {
		echo ', ' . $querylisting['multiemails'];
		} ?>
		</div>
	...
	<tr width="100%">
		<th width="25%" align="left" class="header">
			<div align="left">
				...
				<?php if ($resultpage['moduleType'] == 3) { ?>Fixed&nbsp;Departure&nbsp;ID. <?php } ?>
				<?php if ($resultpage['moduleType'] == 4) { ?>Package&nbsp;ID. <?php } ?>
			</div>
		</th>
	...
	while ($resultlists = mysqli_fetch_array($rs[0])) {
		...
		if (mysqli_num_rows($numQutationQuery) < 1) {
	...
	<tr style=" <?php if ($resultlists['lostStatus'] == 1) { ?>background-color: #b46767; border: 3px solid #b46767; color: #ffffff !important; <?php } elseif ($resultlists['status'] == 1) { ?> background-color: #deffe0; border: 3px solid #6cc791;<?php } elseif ($resultlists['status'] == 2) { ?><?php } elseif ($resultpage['queryStatus'] == 20) { ?> background-color: #c75858;<?php } ?> ">
		<td align="left">
			<div align="left">
				<a <?php if ($resultpage['queryStatus'] == 20 || $resultlists['lostStatus'] == 1 || ($resultlists['queryType'] == 3 && $resultpage['moduleType'] == 1)) { ?> href="showpage.crm?module=quotations&view=yes&id=<?php echo encode($resultlists['id']); ?>" <?php } else {
					if ($resultlists['status'] != 1 ) { ?> href="showpage.crm?module=quotations&view=yes&id=<?php echo encode($resultlists['id']); ?>" <?php } else { ?> onclick="alert('Final Proposal can\'t be edited.');" <?php }
					} ?>>
					<?php
		...
		<?php } ?>
		<td width="10%" align="left"><?php if ($resultpage['dayWise'] == 1  || ($resultpage['moduleType'] == 2 && $resultlists['isSeries'] == 1) || ($resultpage['moduleType'] == 3 && $resultlists['isFD'] == 1) || ($resultpage['moduleType'] == 4 && $resultlists['isPackage'] == 1)) { ?> <div align="left"><?php echo date('d-m-Y', strtotime($resultlists['fromDate'])); ?></div><?php } ?></td>
		<td width="10%" align="left">
			<?php if ($resultpage['dayWise'] == 1 || ($resultpage['moduleType'] == 2 && $resultlists['isSeries'] == 1) || ($resultpage['moduleType'] == 3 && $resultlists['isFD'] == 1) || ($resultpage['moduleType'] == 4 && $resultlists['isPackage'] == 1)) { ?> <div align="left"><?php echo date('d-m-Y', strtotime($resultlists['toDate'])); ?></div><?php } ?></td>
			...
			<td width="8%" align="left">
				<?php if ($resultpage['moduleType'] == 1) { ?>
				<?php if ($resultlists['quotationType'] == 2) { ?>
				<a onClick="alertspopupopen('action=selectCostSheet&quotationId=<?php echo $resultlists['id']; ?>','300px','auto');">
					<div class="saveasbtn" style="margin-right: 5px;">Cost&nbsp;Sheet</div>
				</a>
				<?php } else { ?>
				<a onClick="alertspopupopen('action=addCostSheet&queryId=<?php echo $_REQUEST['id']; ?>&quotationId=<?php echo $resultlists['id']; ?>','1300px','auto');">
					<div class="saveasbtn" style="margin-right: 5px;">Cost&nbsp;Sheet</div>
				</a>
				<?php }
				} ?>
			...
			<td>
				...
				<a href="#" onclick="alertspopupopen('action=addCostSheet&queryId=<?php echo $_REQUEST['id']; ?>&quotationId=<?php echo $resultlists['id']; ?>','1300px','auto');">
					<div class="saveasbtn" style="border-radius: 4px !important;color: #ffffff !important;background-color: #0c0c0c !important;margin-right: 5px;">Package&nbsp;Costing</div>
				</a>
				<?php } ?>
			</td>
			<?php } ?>
			...
			<!-- preview File -->
			<td width="8%" align="left">
				<a href="<?php echo $fullurl; ?>PreviewFiles/crm_proposal.php?propNum=<?php if($resultlists['quotationType']==2){ echo "6"; }else{ echo "4";} ?>&q_token=<?php echo trim($resultlists['q_token']);?>&id=<?php echo encode($resultlists['id']);?>" target="_blank">
			...
			<td width="9%" align="left">
				...
				<div class="saveasbtn" style="margin-right: 5px; color: #fff !important; border: 1px #e17c00 solid !important; background-color: #e17c00 !important;" onClick="alertspopupopen('action=finalquote&queryId=<?php echo $_REQUEST['id']; ?>&quotationId=<?php echo $resultlists['id']; ?>','1000px','auto');">Final&nbsp;Quote</div>
				<?php } else {
				if ($resultpage['queryType'] != 4) { ?>
			...
			<td width="10%" align="left">
				...
				<a <?php if ($resultpage['queryStatus'] != 3) { ?> onClick="alertspopupopen('action=finalQuotation&status=1&queryId=<?php echo $resultpage['id']; ?>&id=<?php echo $resultlists['id']; ?>','300px','auto');" <?php } ?> class="saveasbtn2">Make&nbsp;Final</a>
				<?php
					}
				}
				?>
			...
			<td width="17%" align="left"><?php
				if ($resultlists['isPaymentRequest'] == 1 && $resultlists['status'] == 1) {
					$result = mysqli_query(db(), "select * from " . _PAYMENT_REQUEST_SECTION_MASTER_ . " where queryid='" . $resultpage['id'] . "' and quotationId='" . $resultlists['id'] . "' and deletestatus!=1")  or die(mysqli_error(db()));
					$number = mysqli_num_rows($result);
					$getpaymentid = mysqli_fetch_array($result);
					if ($number > 0) {
						$rsasd = GetPageRecord('id', _PAYMENT_REQUEST_SECTION_MASTER_, 'queryid=' . $resultpage['id'] . ' and quotationId=' . $resultlists['id'] . '');
						$resultpaymentpage = mysqli_fetch_array($rsasd);
					?><a style="color: #e87e11 !important; font-weight: 500; border: 1px solid #4CAF50; padding: 5px; border-radius: 3px; background-color: #ffff;" href="showpage.crm?module=paymentrequest&view=yes&id=<?php echo  encode($resultpaymentpage['id']); ?>&QueryId=<?php echo $resultpage['id']; ?>">Payment&nbsp;Information</a>
				<?php
				} else {
				?>
				<a style="color: #e87e11 !important; font-weight: 500; border: 1px solid #4CAF50; padding: 5px; border-radius: 3px; background-color: #ffff;" href="frm_action.crm?addpaymentrequest=1&quotationId=<?php echo ($resultlists['id']); ?>&QueryId=<?php echo ($resultpage['id']); ?>">Payment&nbsp;Information</a>
				<?php
					}
				}
				?>
	...
	<tbody>
		...
		<tr <?php if ($resultQuery['lostStatus'] == 1) { ?> style="background-color: #b46767; border: 3px solid #b46767; color: #ffffff !important;" <?php }
			...
			<td width="14%" align="left"><?php
											$result = mysqli_query(db(), "select * from " . _PAYMENT_REQUEST_SECTION_MASTER_ . " where queryid='" . $resultQuery['queryId'] . "' and quotationId='" . $resultQuery['id'] . "' and deletestatus!=1")  or die(mysqli_error(db()));
											$number = mysqli_num_rows($result);
											$getpaymentid = mysqli_fetch_array($result);
											if ($number > 0) {
												$rsasd = GetPageRecord('id', _PAYMENT_REQUEST_SECTION_MASTER_, 'queryid=' . $resultQuery['queryId'] . ' and quotationId="' . $resultQuery['id'] . '"');
												$resultpaymentpage = mysqli_fetch_array($rsasd);
	...
	<?php
	$result = mysqli_query(db(), "select * from " . _PAYMENT_REQUEST_SECTION_MASTER_ . " where queryid='" . $resultpage['id'] . "' and deletestatus!=1")  or die(mysqli_error(db()));
	$number = mysqli_num_rows($result);
	$getpaymentid = mysqli_fetch_array($result);
	if ($number > 0) {
...
<!-- Expense entry code ended -->
	...
	if ($diffrence > 0) {
		$select = '*';
		$where = 'queryid=' . $resultpage['id'] . '';
		$rs = GetPageRecord($select, _PAYMENT_REQUEST_SECTION_MASTER_, $where);
	...
	<?php } ?>
	<?php
	$result = mysqli_query(db(), "select * from " . _PAYMENT_REQUEST_SECTION_MASTER_ . " where queryid='" . $resultpage['id'] . "' ")  or die(mysqli_error(db()));
	$number = mysqli_num_rows($result);
	$getpaymentid = mysqli_fetch_array($result);
	if ($number > 0) {
	...
	<a onclick="alertspopupopen('action=assignafterconfirmation&queryId=<?php echo $_GET['id']; ?>','450px','auto');"><i class="fa fa-plus-square dropbtn" aria-hidden="true" style="color:#82b767; cursor:pointer;"></i>Assign&nbsp;User <?php if ($resultpage['confirmedAssignTo'] != 0) { ?> <span style="position: absolute; top: 6px; margin-left: 10px; background-color: #7a96ff; color: #fff; padding: 5px 10px; font-size: 11px; border-radius: 4px; text-align: center;"><?php echo getUserName($resultpage['confirmedAssignTo']); ?></span> <?php } ?></a>
	...
	if ($resultpage['quotationYes'] == 2 && $resultpageQuotation['id'] != '') { ?>
	...
	<tr>
		<td width="33%" align="left" valign="top">
			<div class="clientandsupplierpaymentouter" style="border:1px solid #23ab69;">
				<div class="amount" style="color:#23ab69;"><?php if($requesetdata['finalCost'] == '') {
																	echo '0';
																}else{
																	echo $requesetdata['finalCost'];
				} ?></div>
		...
		<td width="33%" valign="top">
			<div class="clientandsupplierpaymentouter" style="border:1px solid #2374ab;">
				<div class="costtabsbox">
					<div class="amount" style="color:#2374ab;"><?php if ( $TotalrecievedAmount == '') {
																		echo '0';
																	} else {
																		echo $TotalrecievedAmount;
					} ?></div>
		...
		<td width="33%" valign="top">
			<div class="clientandsupplierpaymentouter" style="border:1px solid #f17f00;">
				<div class="costtabsbox">
					<div class="amount" style="color:#f17f00;"><?php if ($requesetdata['pendingCost'] == '') {
																		echo '0';
																	} else {
																		echo $requesetdata['pendingCost'];
					} ?></div>
	...
	<tr>
		<td align="center" valign="top" style="font-size:13px;">
			<?php
			$rsfn = GetPageRecord('*', _QUOTATION_MASTER_, 'queryId="' . decode($_GET['id']) . '" and status=1 and isPaymentRequest = 1');
			$quotationDataFinal = mysqli_num_rows($rsfn);
			?>
			<div class="confirmstatuschangeicon paddingclass <?php if ($resultpage['queryStatus'] == 3) { ?>paddingclassactive<?php } ?>" title="Query Confirmed" <?php if ($resultpage['queryStatus'] != 3 && $quotationDataFinal > 0) { ?> onClick="alertspopupopen('action=queryStatus&queryId=<?php echo $resultpage['id']; ?>&status=3','450px','auto');" <?php } else { ?> style="cursor:no-drop;" <?php } ?>>Query<br />Confirmed</div>
		</td>
		<td align="center" valign="top">
			<div class="sentstatuschangeicon paddingclass  <?php if ($resultpage['queryStatus'] == 6) { ?>paddingclassactive<?php } ?>" title="Option Sent" style="cursor:pointer;" <?php if ($resultpage['queryStatus'] != 3) { ?> onclick="alertspopupopen('action=queryStatus&queryId=<?php echo $resultpage['id']; ?>&status=6','450px','auto');" <?php } ?>>Option<br />Sent</div>
		</td>
		<td align="center" valign="top">
			<div class="timelimitstatuschangeicon paddingclass <?php if ($resultpage['queryStatus'] == 5) { ?>paddingclassactive<?php } ?>" title="Quotation&nbsp;Generated" style="cursor:pointer; background-color:#a598d9 !important;">Quotation<br />Generated</div>
			<!-- <?php //if ($resultpage['queryStatus'] != 3) { ?> onclick="alertspopupopen('action=queryStatus&queryId=<?php //echo $resultpage['id']; ?>&status=5','450px','auto');" <?Php //} ?> -->
		</td>
		<td align="center" valign="top">
			<div class="loststatuschangeicon paddingclass  <?php if ($resultpage['queryStatus'] == 4) { ?>paddingclassactive<?php } ?>" title="Query Lost" <?php if ($resultpage['queryStatus'] != 3) { ?> onclick="alertspopupopen('action=queryStatus&queryId=<?php echo $resultpage['id']; ?>&status=4','450px','auto');" <?php  } ?> style="cursor:pointer;">Query<br />Lost</div>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="4" style="font-weight: 500; font-size: 14px; font-weight:500 !important;">Notes & Reminder</td>
	</tr>
	<tr>
		<td align="center" valign="top">
			<div class="bookwithme paddingclass" onClick="alertspopupopen('action=addnotesinquery&queryId=<?php echo $resultpage['id']; ?>&type=2','600px','auto');" style="cursor:pointer;">Internal<br />Note's</div>
		</td>
		<td align="center" valign="top" style="font-size:13px;">
			<div class="notreachable paddingclass" onClick="alertspopupopen('action=addnotesinquery&queryId=<?php echo $resultpage['id']; ?>&type=1&assignTo=<?php echo $resultpage['assignTo'] ?>','600px','auto');" style="cursor:pointer;">Not<br />reachable</div>
		</td>
		<td align="center" valign="top">
			<div class="talkprogress paddingclass" style="cursor:pointer;" onClick="alertspopupopen('action=addnotesinquery&queryId=<?php echo $resultpage['id']; ?>&type=3','600px','auto');">Talk&nbsp;in<br />progress</div>
		</td>
		<td align="center" valign="top">
			<div class="finalyze paddingclass" style="cursor:pointer;" onClick="alertspopupopen('action=addnotesinquery&queryId=<?php echo $resultpage['id']; ?>&type=4','600px','auto');">Finalizing<br />soon</div>
		</td>
	</tr>
	<?php if ($resultpage['queryStatus'] != 20) { ?>
	<tr>
		<td align="center" valign="top" style="font-size:13px;">
			<div class="notreachable paddingclass" onClick="alertspopupopen('action=cancelvoucher&queryId=<?php echo encode($resultpage['id']); ?>','600px','auto');" style="cursor:pointer;">Query<br />Cancel</div>
		</td>
		<td colspan="3" style="font-weight: 500;font-size: 14px;">&nbsp;</td>
	</tr>
	<?php }  ?>
	</tr>
	</table>
	...
	<?php if ($resultpage['queryStatus'] == 2 || $resultpage['queryStatus'] == 1 || $resultpage['queryStatus'] == 10) {
	} else { ?>
	Remarks
	<?php } ?>
	</div><?php } ?>
