...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
   <td width="7%">
     ...
     <!--<td >
       <button name="addnewuserbtn" type="button"  style="background-color:#fff!important;border:2px solid gray;border-radius:50%;color:#000;padding:7px;width:50px;margin-left:10px;cursor:pointer" onclick="window.history.back();" /><i class="fa fa-arrow-left" style="font-size:24px"></i>
        </button>    
     </td> --> 
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=City','600px','auto');" />
	<?php } ?>
	</div>
...
<input name="action" id="action" type="hidden" value="deletecity" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
   ...
   <tr>
     <th width="1%" class="header">Sr</th>
     <th width="2%" height="28" align="center" valign="middle" class="header" ><?php if($editpermission==1){ ?> <input type="checkbox" id="checkAll"  name="checkedAll" onclick="checkallbox();" /><?php } ?>
       <label for="checkAll"><span></span>&nbsp;</label>
      </th>
       <th height="28" align="left" class="header" >City Name </th>
      <th align="left" class="header" >Country Name</th>
     <th align="left" class="header" >State Name </th>
...
$rs=GetRecordList($select,_CITY_MASTER_,$where,$limit,$page,$targetpage); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
$country_result=mysqli_fetch_array($rs1);

 
$rs1d=GetPageRecord('*','countryMaster','id="'.$country_result['countryId'].'"'); 
...
?>
  <tr>
    ...
    <td width="5%" height="28" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div></td>
    <td width="5%" align="left"><div class="bluelink"><?php echo $resCountry['name']; ?></div></td>
	 <td width="5%" align="left"><div class="bluelink"><?php echo $country_result['name']; ?></div></td>
