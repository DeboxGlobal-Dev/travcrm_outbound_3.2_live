 ...
 $where2 = 'mobile_number="' . $mobile . '" and queryId ="'.$queryId.'" ORDER BY id DESC';
    ...
    // print_r($guestFolder);die();
     ...
     if (!empty($_FILES['img1']['name'])) {
        $destinationDirectory = '../../../docFiles/TourCodes/'.$queryIdFolder.'/'.$guestFolder.'/pancard/';
        
        $fileExtension = pathinfo($_FILES["img1"]['name'], PATHINFO_EXTENSION);

        // $uploadFileName = time() . str_replace(' ', '', $_FILES['img1']['name']);
        $pancard_attachment1 = $destinationDirectory.time().'-pancard-'.$guestFolder.'.'.$fileExtension;

        // $uploadimg1 = $destinationDirectory . $uploadFileName;
        
        $pancard_attachment = str_replace('../../../', '', $pancard_attachment1);
        if (!file_exists($destinationDirectory)) {
            mkdir($destinationDirectory, 0755, true);
        }
        if (move_uploaded_file($_FILES['img1']['tmp_name'], $pancard_attachment1)) {
            echo "File uploaded successfully: $pancard_attachment";
        } else {
  ...
  $documenttype = "UPDATE `mice_guestListMaster` SET `pan_attachment` = '$pancard_attachment',`pan_status`=1,`pan_number`='$panNo' WHERE $namevalue";
