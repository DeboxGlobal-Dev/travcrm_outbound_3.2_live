	...
	if($_REQUEST['report'] == '5'){ 
		...
		while($resultlists=mysqli_fetch_array($rs[0])){ 
	        ...
	        if ($diffrence > 0) {
				...
				$wherep='id='.$idp.'';
				$rsp=GetPageRecord($selectp,_PAYMENT_REQUEST_MASTER_,$wherep);
				...
				// Contact_Number 
				$Contact_Number = getPrimaryPhone($suppid,'suppliers');
	...
	if($_REQUEST['report'] == '8'){
		...
		while($resultlists=mysqli_fetch_array($rs[0])){
			...
			if($clientType==2){
				...
				// mobile
			   	$mobile =  getPrimaryPhone($contantnamemain2['id'],'contacts');
				// email
			   	$email =  getPrimaryEmail($contantnamemain2['id'],'contacts'); 
			}
			else{
			  	$select22='*';  
			  	$where22='id='.$resultlists['companyId'].''; 
				...
				// mobile
			   	$mobile =  getPrimaryPhone($contantnamemain2['id'],'corporate');
				// email
			   	$email =  getPrimaryEmail($contantnamemain2['id'],'corporate');
			}
...
if($_REQUEST['report']==21){
	...
	if($fromDate == '01-01-1970' && $toDate == '01-01-1970'){
		...
		$strWhere=' and  Q.queryDate BETWEEN \"".$fromDate."\" and \"".$toDate."\" and Q.deletestatus=0 ';
		//$dateCon = ' and queryMaster.queryDate BETWEEN "'.$fromDate.'" and "'.$toDate.'" and queryMaster.deletestatus=0 ';
		$dateCon = ' and Q.queryDate BETWEEN "'.date('Y-m-d', strtotime( $_REQUEST['fromDate'] )).'" and "'.date('Y-m-d', strtotime( $_REQUEST['toDate'] )).'" and Q.deletestatus=0 ';
	}else{
		$dateCon = ' and Q.queryDate BETWEEN "'.date('Y-m-d', strtotime( $_REQUEST['fromDate'] )).'" and "'.date('Y-m-d', strtotime( $_REQUEST['toDate'] )).'" and Q.deletestatus=0 ';
	}
...
$DataQuery .= " ( ";
$DataQuery .="SELECT QI.serviceType AS ServiceType,QI.startDate AS StartDate,PBHM.hotelName AS ServiceName,Q.clientType AS ClientType,Q.companyId AS Companyid,Q.leadPaxName AS LeadpaxName,Q.adult AS Adult,Q.assignTo AS AssignTo,DM.name AS Destination,Q.id AS QueryId from quotationItinerary QI INNER join quotationHotelMaster QHM on QI.quotationId=QHM.quotationId and QI.serviceId=QHM.id INNER JOIN quotationMaster QM on QI.quotationId=QM.id and QM.status=1 INNER JOIN queryMaster Q on QM.queryId=Q.id ".$dateCon." INNER JOIN packageBuilderHotelMaster PBHM on QHM.supplierId=PBHM.id INNER JOIN destinationMaster DM on QHM.destinationId=DM.id where QI.serviceType='hotel'";
$DataQuery .= " UNION all " ;
$DataQuery .="SELECT QI.serviceType AS ServiceType,QI.startDate AS StartDate,PBTM.transferName AS ServiceName,Q.clientType AS ClientType,Q.companyId AS Companyid,Q.leadPaxName AS LeadpaxName,Q.adult AS Adult,Q.assignTo AS AssignTo,DM.name AS Destination,Q.id AS QueryId from quotationItinerary QI inner JOIN quotationTransferMaster QTM on QTM.id=QI.serviceId and QTM.quotationId=QI.quotationId INNER JOIN quotationMaster QM on QI.quotationId=QM.id and QM.status=1 INNER JOIN queryMaster Q on QM.queryId=Q.id ".$dateCon." inner join vehicleMaster VM on QTM.vehicleModelId = VM.id inner join packageBuilderTransportMaster PBTM on QTM.transferNameId = PBTM.id INNER JOIN destinationMaster DM on QTM.destinationId=DM.id where QI.serviceType in ('transfer','transportation')";
...
$rs=GetPageRecord('*',_QUOTATION_TRANSFER_MASTER_,' 1 and queryId in (select id from queryMaster where queryStatus=3 '.$dateCon.') order by id asc '); 
...
$rs=GetPageRecord('*',_QUOTATION_GUIDE_MASTER_,' 1 and tariffId!=0 '.$dateFandT.' order by quotationId desc '); 
while($resultlists=mysqli_fetch_array($rs)){
