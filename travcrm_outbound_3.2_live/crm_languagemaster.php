...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <?php if($deletepermission==1){ ?> 
	...
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopen('action=mastersdelete&name=Language','600px','auto');" />
	<?php } ?>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
       ...
       <?php 
        $rs=GetPageRecord('*','tbl_languagemaster','1 and name!="" and status=1 and deletestatus=0');
        if(mysqli_num_rows($rs) < 10){        
         if($addpermission==1){
          ?>
          <td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add <?php echo $pageName; ?>" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> 
        <?php } }  ?>
      </tr>
...
<input name="action" id="action" type="hidden" value="delete_language_" />
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
  <thead>
    <tr>
      <th width="2%" align="left" class="left">Sr.</th>    
      <th width="2%" align="center" valign="middle" class="header" > <?php if($editpermission==1){ ?>
          <input type="checkbox" id="checkAll"  name="checkedAll" onclick="checkallbox();" />
          <?php } ?>
          <label for="checkAll"><span></span>&nbsp;</label>      </th>
      <th align="left" class="header" width="15%" >Language Value</th>    
      <th align="left" class="header" width="15%">Language</th>
      <th align="left" class="header" width="15%">Created By</th>
      <th  align="left" class="header" width="15%">Modified By </th>
      <th  align="left" class="header" width="15%">Status </th>
      </tr>
  </thead>
  <tbody>
    <?php
...
// $where='where name!="" and deletestatus=0 ORDER BY `name` ASC'; 
// $page=$_GET['page'];
...
$rs=GetRecordList($select,'tbl_languagemaster',$where,$limit,$page,$targetpage ); 
$totalentry=$rs[1]; 
$paging=$rs[2]; 
...
?>
    <tr>
      ...
      <td align="center" width="2%" valign="middle"><?php if($editpermission==1){ ?>
        ...
        <?php } ?></td>
        <td align="left" width="15%"><?php echo 'lang_0'.$resultlists['id']; ?></td>
      <td width="15%" align="left"><div class="bluelink" onclick="masters_alertspopupopen('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div></td>
