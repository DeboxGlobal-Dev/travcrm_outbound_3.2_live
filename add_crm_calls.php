...
deleteRecord(_CALLS_MASTER_,$wheredel);
...
$lastqueryidmain= addlistinggetlastid(_CALLS_MASTER_,$namevalue);
}
...
<option value="2" <?php if(2==$clientType){ ?>selected="selected"<?php } ?>>B2C</option> 
	...
	<div class="griddiv" id="selectclientbox" style="overflow: unset;">
		<div onclick="openselectCompanypop('action=addAgentClienttoQuery&actionType=addserviceagentclient&addTo=calls','700px');" style="position: absolute;cursor: pointer;top: 22px;right: 3px;background: #233a49;color: white;padding: 8px 5px;">Add New</div>
...
$start=strtotime('00:00');
   ...
   <option value="<?php echo date('g:i A',$i); ?>" <?php if('9:00 AM'==date('g:i A',$i) && $_REQUEST['id']==''){ ?> selected="selected"<?php } ?> <?php if($editstarttime==date('g:i A',$i)){ ?> selected="selected"<?php } ?>><?php echo date('g:i A',$i); ?></option>;
	...
	<div class="griddiv">
	<label>
	<div class="gridlable">Call&nbsp;Duration</div>
	<select id="timeDuration" name="duration" class="gridfield validate" displayname="Call Duration" autocomplete="off" onchange="setendtime(this.value);"> 
<option value="">Select</option>
<?php
...
<?php } ?>
	...
	<div class="griddiv">
	<label>
	<div class="gridlable">Call&nbsp;Type</div>
	<select id="directiontype" name="directiontype" class="gridfield" displayname="Call Type" autocomplete="off"  >
	 <option value="">Select</option> 
...
$select='*';   
$where='id!="" order by id'; 
$rs=GetPageRecord($select,_DIRECTION_TYPE_MASTER_,$where); 
while($rest=mysqli_fetch_array($rs)){ 
...
<?php } ?>
	...
	<div class="griddiv"><label>
	<div class="gridlable">Call&nbsp;Agenda<span class="redmind"></span></div>
