...
<div class="rightsectionheader"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td><div class="headingm" style="margin-left:-10px;"><span id="topheadingmain"><?php echo 'Vehicle Brand ';//$pageName; ?></span>
	<div id="deactivatebtn" style="display:none;">
	 <?php if($deletepermission==1){ ?> 
	
	<input name="deactivate" type="button" class="redmbutton" id="deactivate" value="Deactivate" onclick="masters_alertspopupopenvehicle('action=mastersdelete&name=vehiclebrandmaster','600px','auto');" />
	<?php } ?>
	</div>
    ...
    <td align="right"><table border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td><input name="keyword" type="text" class="topsearchfiledmain" id="keyword" style="width:150px;" value="<?php if(isset($_GET['keyword'])){echo $_GET['keyword']; } ?>" size="100" maxlength="100" placeholder="Brand Name"></td>
        ...
        <?php if($addpermission==1){ ?><td style="padding-right:20px;"><input name="addnewuserbtn" type="button" class="bluembutton" id="addnewuserbtn" value="+ Add Brand" onclick="masters_alertspopupopenvehicle('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>','400px','auto');" /></td> <?php } ?>
      </tr>
  ...
  <input name="action" id="action" type="hidden" value="deletevehiclebrand" />
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped table-bordered" id="mainsectiontable">
      ...
      <tr>
        ...
        <label for="checkAll"><span></span>&nbsp;</label></th> 
          <th align="left" class="header" >Vehicle Type </th>
          <th align="left" class="header" >Brand Name </th>
      <!-- <th  align="left" class="header" >Vehicle Image</th> -->
  ...
  <?php
    ...
    $rs=GetRecordList($select,_VEHICLE_BRAND_MASTER_,$where,$limit,$page,$targetpage); 
    $totalentry=$rs[1]; 
    $paging=$rs[2]; 
    ...
    ?>
      <tr>
        <td width="2%"><?=$no?></td> 
        <td align="center" width="2%" valign="middle"><?php if($editpermission==1){ ?><input type="checkbox" id="c<?php echo $no; ?>" name="check_list[]" class="chk"  value="<?php echo encode($resultlists['id']).','.$resultlists['status']; ?>"/>
        <label for="c<?php echo $no; ?>"><span></span>&nbsp;</label><?php } ?></td>
        <td width="11%" align="left"><div class="bluelink" onclick="masters_alertspopupopenvehicle('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo getVehicleTypeName($resultlists['vehicleType']);  ?></div></td>
        <td width="11%" align="left"><div class="bluelink" onclick="masters_alertspopupopenvehicle('action=addedit_<?php echo clean($_GET['module']); ?>&sectiontype=<?php echo clean($_GET['module']); ?>&id=<?php echo $resultlists['id']; ?>','400px','auto');" ><?php echo $resultlists['name']; ?></div>   </td>
      
    <!-- <td width="17%" align="left"><?php if($resultlists['image']!=''){ ?><img src="<?php echo $fullurl; ?>packageimages/<?php echo $resultlists['image']; ?>" style="width: 150px;" alt="<?php echo $resultlists['name']; ?>" /> <?php } ?></td> -->
